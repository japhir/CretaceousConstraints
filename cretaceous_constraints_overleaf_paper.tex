...
% inline shows tracked changes
% NOTE: I've tweaked the .sty file so that default editor becomes blue.
\usepackage{soul} % hyphenable spacing, underlining, striking out etc. Needed for trackchanges.

% IJK added packages
% hyperref needs to be added before apacite, so cannot add here or refs will break
% \usepackage{hyperref} % links between references etc
...
\newacronym{RCSD}{RCSD\(_{i}\)}{root cumulative sum square deviation}
% do we need RCSSD?
...
% \newacronym{KT}{KPB}{Cretaceous--Paleogene boundary}
...
\title{A\change{n}{ Revised} Astronomically Tuned Time Scale for the Latest Cretaceous (Maastrichtian)}
...
is variable between different astronomical solutions due \add{to} resonance transitions related to \(g_{4}\) and \(g_{3}\), which are ``loosely related to the perihelion precession of Earth's and Mars' orbits''~\cite{ZeebeLourens2022EPSL}.
...
Chaotic resonance transitions can change the timing of these \glspl{VLN} so that they occur with a variety of periods, for example \appr\qty{1.2}{\millionyear}~\cite{Laskar2011a,ZeebeLourens2019,ZeebeLantink2024,Olsen2019}\note{added Olsen2019}.
...
For example, the long eccentricity minimum near \qty{65.9}{\millionyearago} differs by \remove{up to 68}\qty{70}{\kiloyear} between solutions La11 and ZB20a~\cite<recalculated after>{ZeebeLourens2022EPSL}.
\add{Note that the age of this eccentricity minimum for solution La10d differs by} \qty{75}{\kiloyear} \add{from the age of the ZB20a solution. La10c (}\qty{34}{\kiloyear}\add{) and L10b (}\qty{33}{\kiloyear}\add{) and to a lesser extent La10a (}\qty{23}{\kiloyear}\add{) also have a minimum that is older than the ZB18a}  (\qty{10}{\kiloyear}\add{) and all ZB20x solutions(}\qtylist{0;3;10;10}{\kiloyear}\add{, a--d respectively).}
% \ijk{TODO: NT, Furthermore, solutions La10 a, b, c, and d also differ. Above OK or too detailed?}
This means that providing absolute ages \change{of}{for} Maastrichtian records based on tuning to the long eccentricity cycle of any particular astronomical solution %without first constraining the chaos in this interval with geological records,
...
\change{field}{land-based marine} sections,
...
while the upper Maastrichtian \add{succession} was \add{deposited under} tectonically more stable \add{conditions}~\cite{Pujalte1999}.
...
\change{Bundles of five of these couplets with darker, redder, thicker beds---especially in the marls---were related to short eccentricity amplitude modulation}{The couplets occured in bundles of five, typically consisting of 3--4 darker, thicker marls that are bounded by intervals of 1--3 couplets with thinner, carbonate-rich marls.}
%(\appr\qty{4}{\metre}, \appr\qty{100}{\kiloyear}),
\change{which in turn were grouped in four bundles that were associated with the long eccentricity cycle and expressed as limited thinner marlier parts}{The couplets with the most distinct difference between the lithologies (dark marls) likely represent the highest amplitude of the forcing signal during eccentricity maxima, whereas limited lithological contrast (thin marls) can indicate eccentricity minima}~\cite{Batenburg2012,Batenburg2014}.
...
The \gls{d13C} proxy showed long eccentricity-related cycles \add{(where eccentricity minima coincide with} \gls{d13C} \add{maxima}), but the short eccentricity cycles were less clearly expressed.
...
\add{Minima in} \gls{MS} \add{were previously linked to eccentricity minima}~\cite{Batenburg2012}.
...
\add{Higher} \gls{L*} \add{values are linked to eccentricity minima.}
We therefore argue that the color reflectance \gls{L*} record most directly reflects the \add{local} orbital forcing.
...
The original study relied on foraminifera biostratigraphy, magnetostratigraphy, and astrochronology \change{by tuning}{to tune} the long eccentricity minima to the long eccentricity filter of the La11 solution~\cite{Laskar2011a}. % not sure if sentence is needed
...
\begin{enumerate}
    ...
    \item Fit a \add{smooth line through the record, using a } \gls{GAM} \add{(with a penalized cubic regression spline, i.e. R-code \texttt{mgcv::gam(y \textasciitilde{} s(x, bs="cs"), method = "REML", data = dat)}, the default of \texttt{ggplot2::geom\_smooth()} for a large number of datapoints)}, subtracted the fit from each value, then scaled the results;
    % \ijk{TODO: is this GAM description OK now?}
...
\add{
We also calculate a cumulative version of the} \gls{RMSD} \add{to investigate the temporal evolution of the match, which we label the} \gls{RCSD}:
\note{added new acronym and equation}
\begin{equation}
\text{RCSD}_{i} = \sqrt{\sum_{i=1}^{k}(e_{i} - f_{i})^{2}},
\end{equation}
\add{where} \(k\) \add{is the total number of differences with the interpolated data.}
...
\remove{We also calculated a square root of the cumulative sum of the squared differences that started at the KTB and moved to older data points.
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{Lstar-vs-solutions_grayMa405.png}
  \caption{\label{fig:rolling-depth-age}
    ...
    \note{added gray background for 5, 8, 10.}
...
% DONE: double-check the cycle counts between the two studies
...
\add{The resulting age model is shown in} \cref{tab:timescale}\add{.}
...
\add{The ages of our updated time scale (}\cref{tab:timescale}\add{) were on average} \appr\qty{69}{\kiloyear} \add{younger than the original age model} \cite{Batenburg2012}\add{, which was based on the La11 solution.
However, this difference mostly arises because of differences between the La11 and ZB20a solutions.
If we apply the ZB20a solution without tie-point adjustments, the average age of the tie-points would be only} \qty{13.2}{\kiloyear} \add{older.}
\add{While our algorithm allowed shifting the record by} \qty{\pm200}{\kiloyear}\add{, the best-fit for ZB20a shifted the record by} \qty{-6.4}{\kiloyear} \add{for Zumaia and} \qty{-9.6}{\kiloyear} \add{for Sopelana.
For La10b and La10c, the filtered record was shifted by} \qtylist{123;132}{\kiloyear} \add{respectively for Sopelana (}\qtylist{6;9}{\kiloyear}\add{ for Zumaia) while the records were shifted by less than} \qty{52}{\kiloyear} \add{for both Zumaia and Sopelana for the other astronomical solutions analyzed in this study.}
% TODO: is the above clear enough?
% - explain differences from original age model
% - explain by how much we had to shift the record prior to optimizing the tie-points.
...
The \remove{cumulative RMSD}\acrfull{RCSD} scores indicate when our filtered records diverged from the astronomical solutions (\cref{fig:cum-RMSD-all}).
Solutions ZB18a, ZB20c, and ZB20d all show a rapid increase in the \remove{cumulative RMSD}\gls{RCSD} score at around \qty{65.9}{\millionyearago}, which represents the mismatch in the first short eccentricity cycle just prior to the \gls{KT}.
...
Unsurprisingly, the \remove{cumulative RMSD}\gls{RCSD} scores highlight that none of the astronomical solutions showed an amplitude that is as large as the filtered record in \ma{8}, resulting in a rapid increase of the \remove{cumulative RMSD}\gls{RCSD} scores for all astronomical solutions under evaluation for this interval.
...
The \remove{cumulative RMSD}\gls{RCSD} score only increased slightly in \ma{10} for ZB20b, whereas there appears a clear coincidence of a \gls{VLN} in the solution paired with a relatively large amplitude in the filtered record.
...
During \ma{10}, the \remove{cumulative RMSD}\gls{RCSD} score for ZB20a steadily increases at a slower rate, and therefore results in a better overall match.
...
have a great deal of influence on the \remove{cumulative RMSD}\gls{RCSD} scores.


% FIGURE 3
\begin{figure}
  \centering
  \includegraphics[width=0.6\textwidth]{cumulative_rRMSD_all.pdf}
  \caption{\label{fig:cum-RMSD-all}
    \textbf{Best matches through time.}
    \note{updated remainder of figure caption.}
...
% \ijk{TODO: NT add notes on phase relationship of different proxies with insolation forcing! \(\to\) where in the discussion? maybe here?}
...
\add{Maxima in} \gls{L*} \add{and} \gls{d13C} \add{and minima in } \gls{MS} \add{were linked to eccentricity minima.}
Unsurprisingly, none of the astronomical solutions matched perfectly with the proxy archives of the Zumaia and Sopelana \add{outcrops} (as indicated by visual comparison (\cref{fig:rolling-depth-age}), \gls{RMSD} scores (\cref{fig:full-RMSD-all}), and \remove{cumulative RMSD}\gls{RCSD} scores (\cref{fig:cum-RMSD-all}))\note{added figure references}.
...
\add{Overall, these proxy records showed very little spectral power in the obliquity band}~\cite{Batenburg2012}, \add{so intervals with stronger obliquity signals could not be used to verify the locations of} \glspl{VLN}.
...
\add{Comparing rectangular, gaussian, and taner filter window types (further described and illustrated in} \ref{sec:sensitivity}\add{) revealed that the taner filter (as implemented in }\texttt{astrochron::taner()}\add{) gave the best results with a roll parameter of }\(10^{10}\)\add{.}
\add{Experimentation with different ways of filtering the record is recommended, because the optimal parameter choices depend on characteristics of the proxy archive (sampling resolution, peak width, etc.).}
\add{Note that we had to scale the gaussian window parametrization to allow for comparison with similar filter widths (see} \ref{sec:sensitivity} \add{for further details).}
...
One potential caveat to this conclusion is that section photographs from the \change{field}{outcrops} of Zumaia may show indications for a large amplitude of the short eccentricity related cycle in \ma{2}~\cite{Batenburg2012,Dinares-Turell2013}.
...
\add{We also note that the highly regular precession-scale cycles during} \ma{12}~\cite<from the filter output and photographs of>{Batenburg2014} \add{could imply the presence of a VLN at this time.}
\add{Astronomical solution ZB18a has a} \gls{VLN} \add{during this interval, while it seems to occur between} \ma{11} and \ma{12} \add{in ZB20a and between} \ma{12} and \ma{13} \add{in ZB20c.}
\add{Solutions ZB20b, ZB20d, La10b and to a lesser extent La10c, however, show very high amplitude in the long eccentricity cycles during this interval, which would result a more variable expression of precession cycles because eccentricity is the envelope of precession.}
\add{Therefore, our preferred solution ZB20a could still be compatible with the regular precession-scale alternations during} \ma{12}.
...
%of~\citeA{Husson2011}.
% but note that Husson did not convert from MBSF to RMCD
%The key intervals of \ma{5}, \ma{8}, and \ma{10} are \ijk{XXX}.
In conclusion, we recommend using the ZB20a solution for astronomical tuning of eccentricity-related cycles up to \appr\qty{71}{\millionyearago}, because it is the most compatible with available geological records for the Maastrichtian.
...
% TODO: add typical shift here as well
...
As a quality control, we visualized the \gls{RMSD} scores for each \add{long-eccentricity-related} tie-point \add{(1 through 10 from right to left)}\add{ for each} adjustment \add{to the tie-point} depth\add{,} to make sure that a local minimum \add{in the} \gls{RMSD} \add{score} was found\remove{ and that the resulting filtered record did not show any very large changes in sedimentation rate} (see \cref{fig:tiepoint-RMSD-optima} for an example).
% \ijk{TODO: improve this description here! I tried, but am not sure what he struggled to understand so I don't know how to rewrite to be more clear...}
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{MS-vs-solutions_grayMa405.png}
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{d13C-vs-solutions_grayMa405.png}
...
\begin{figure}
  ...
  \caption{\label{fig:tiepoint-RMSD-optima}
    \textbf{Illustration of how RMSD score\remove{s} change\add{s} by performing tie-point depth optimization.}
    \add{(top panel)} The \gls{RMSD} scores as they evolve when each tie-point is iteratively shifted \add{in the depth domain} for the filtered \gls{L*} record of Zumaia \remove{(top panel)} against ZB18a \add{to find the local minimum}.
    \add{(bottom panel)} The filtered record (black) versus ZB18a (yellow)\remove{(bottom panel)}.
    This uses the taner filter from the main text on the piecewise linearly detrended record.
    % \ijk{TODO: is the caption clear enough now?}
    }
\end{figure}
...
\Gls{MS}, \gls{L*}, and \gls{d13C} data used in this study are from \citeA{Batenburg2012,Batenburg2014}.
...
The algorithm R-code \change{will be made}{is} available on the authors' GitHub (\url{https://github.com/japhir/CretaceousConstraints}) and \add{is} archived on Zenodo \cite{Kocken2024a}\remove{upon publication
and is accessible to reviewers via the link: https://surfdrive.surf.nl/files/index.php/s/fGlvE18rNy6hLTG}.
...
\add{We thank N.~Thibault and S.J.~Batenburg for their helpful reviews that improved the manuscript.}
%TODO: do we want to add something like this:
%, which highlighted more observations from the field/data to include in our comparison.
