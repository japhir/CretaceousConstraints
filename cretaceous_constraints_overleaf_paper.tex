...
whose occurrence shows many random spectral peaks, but may also include an eccentricity and obliquity signal for cycles Ma\(_{405}\)1--4~\cite<the first four long eccentricity bundles prior to the \gls{KT}-boundary>{tenKateSprenger1993}. % this part is not mentioned in Batenburg
...
% See also \citeA{Gilabert2022} for comparision of the \citeA{tenKateSprenger1993} \ce{CaCO3} data to \gls{MS}.
% it shows strong negative correlation but with offsets for the pre-/post/transitional \gls{KT} interval.
% Their Fig S2.
...
The long eccentricity cycles are closely matched to each solution, while the short eccentricity filter seems in general alignment with each solution in those intervals where the solution has high short eccentricity power.
Crucially, however, some astronomical solutions have a very low amplitude in the short eccentricity component in contemporaneous intervals where the data show a relatively high amplitude.

The key differentiating factors between astronomical solutions are the locations of the \acrpl{VLN}, where the amplitude of the short eccentricity cycle is very low and the long eccentricity cycle dominates.
This occurs when the individual components that make up the short eccentricity cycle (i.e., \qtylist{95;125}{\kiloyear}) interfere and cancel each outher out, leaving only the long eccentricity component clearly visible.

% \subsection{Very long eccentricity nodes}
Because of the filtering approach it is difficult to distinguish these very long eccentricity nodes in the filtered data directly, however.
There may be changes in sedimentation rate or lithology that result in a low-amplitude in the filtered short eccentricity cycle.
It is more straightforward to detect bundles with a high amplitude in the short eccentricity cycle in the data, that may indicate the absence of a \gls{VLN}.

For example, in bundle \ma{5} (\appr\qty{67.8}{\millionyearago}) ZB20b has a relatively lower amplitude (with a full-fledged \gls{VLN} in \ma{6}) than our eccentricity construct.
In bundle \ma{8} (\appr\qty{68.9}{\millionyearago}), the high amplitude of the short eccentricity filtered records excludes a very long eccentricity node.
This is significant, because solutions La10b, ZB18a, and ZB20c have a low amplitude of short eccentricity in this interval.
The same goes for \ma{10} at \qty{69.7}{\millionyearago}, which does not match ZB20b and to a lesser extent La10c.
