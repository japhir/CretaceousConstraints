...
\begin{table}
    ...
    \begin{tabular}{llccl}
        ...
        342-U1403 & d18O, ln(Fe/Ca) & 1--7 & 68.8--66 & \citeA{Batenburg2018}\\ % low sedrate of ~1 cm / kyr
        ...
        Gubbio & d13C, MS & 1--12 & 76--66 & \citeA{Voigt2012}\\ % low-res
        % Gubbio & Ca isotopes & - & OAE-2 & \citeA{Kitch2022}\\ % low-res, old, wrong proxy
        198-1210B & d13C, d18O, XRF Ba & 2--12 & 71.5--66.25 & \citeA{Jung2012,Kim2022}\\ % some low-res intervals in crucial parts? higher res around 68 Ma, Kim2022 pretty high res but only d13C
        74-525A & grayscale & 6--8.5 & 69--67.8 & \citeA{Husson2011}\\ % have emailed requesting data, but has left academia
        122-762C & grayscale & 8--17+ & 77.5--69 & \citeA{Husson2011,Thibault2012}\\ % Thibault has published most data!
        % the above may be useful to look at, some core gaps but high amplitude short ecc in Ma405_8 and Ma405_10!
        207-1258A & MS & 8--14 & - & \citeA{Husson2011}\\ % can reproduce by getting data from JANUS database
...
We assume a duration of \qty{405}{\kiloyear} for each of these long eccentricity cycles to get a floating age model.
...
%   \texttt{piecewise linear} & %Combination of fine and coarse, where we make the deeper part a single chunk.\\
%        Chop the record into sections based on changes in lithology and visual trends in the proxy records that do not appear cyclic.\\
%   \texttt{gam} & Subtract a \gls{GAM} from the raw value, then scale the result.\\
...
%   \texttt{lowpass} & Subtract a lowpass filter from the value, then scale the result.\\
% \end{tabular}
...
In order to test which astronomical solution provides the best match to the data (end hence a proper astronomical time scale) we adopt and extend the approach of \citeA{ZeebeLourens2019,ZeebeLourens2022EPSL}.
...
% FIGURE 3
% I prefer my new cumsum analysis below :)
\setcounter{figure}{3000}
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.7\textwidth]{cumulative_rRMSD.pdf}
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=\textwidth]{cumulative_rRMSD_all.pdf}
...
% TODO: make separation between detrend_types and filtering strategy!
% \begin{figure}[htb]
%   \centering \includegraphics[width=\textwidth]{RMSD_detrend_types.png}
%   \caption{\label{fig:full-RMSD-detrend}
%     \textbf{Sensitivity analysis for various ways of detrending the data and filtering the short and long eccentricity components.}
%     The rectangular and gaussian filters use the same frequencies as upper and lower filter limits,
%     but the parametrization of the gaussian window effectively narrows the bandpass filter.
%     This is why we also tried several taner filters with changing parameters to get the best overall RMSD scores (not shown)
%     before settling on a roll of \(10^{12}\).
%     % Extended version of \cref{fig:full-RMSD-all}.
%     }
% \end{figure}
