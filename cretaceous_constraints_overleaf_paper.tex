...
See \cref{fig:rolling-rmsd-improvement} for a comparison of \gls{RMSD} scores across all detrend types, taner filters, different proxies, and whether or not we performed our tie-point adjustments (\cref{sec:algorithm}).
See also \cref{fig:rolling-age-MS,fig:rolling-age-d13C} for the fits against the astronomical solutions in the time domain.
% TODO: I removed these figures for now, will remake them cleanly.
...
For the RMSD comparison, we select the best scores across all analyses (see \cref{fig:full-RMSD}).
...
\begin{figure}[htbp]
  ...
  \caption{\label{fig:detrend}
    ...
    % TODO: remove a few of the lin_pred_xxx's
  }
\end{figure}
...
\begin{figure}[htb]
  ...
  \caption{\label{fig:rolling-depth-age-ms}
...
\begin{figure}[htb]
  ...
  \caption{\label{fig:rolling-depth-age-d13C}
