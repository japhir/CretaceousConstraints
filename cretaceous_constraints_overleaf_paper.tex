...
The Sopelana section covers the lower part of the Maastrichtian, and shows similar lithology but has no indications for turbidites, likely because it was located farther offshore~\cite{Batenburg2012}.
...
The \gls{MS} signal is affected by dilution with carbonates~\cite{tenKateSprenger1993}, so its link to climate forcing is less direct that, for example \ce{CaCO3} content itself.
...
Here we assume a duration of \qty{405}{\kiloyear} for each of these long eccentricity cycles to arrive at an initial floating age model.
...
In the main manuscript, we limit the analysis to the simplest 1:1 combination because this preserves the relative amplitude of both components from the original data.
...
\ijk{[@REZ:\ I'm not sure where best to link to the appendix figures that show how the RMSD scores improve after this. Bringing it all up in the results might be repetitive?---asked this in person, better to keep separate for now!]}
...
This allows us to visualize \rez{the time domain in which the data} differs most from the astronomical solutions.
\ijk{TODO:\ talk about \texttt{depth\_chunk}s for different sedimentation rate intervals vs. rolling RMSD? Maybe only in appendix text?}
...
The matched eccentricity construct for the \gls{L*} record visually seemed to match the different astronomical solutions quite well (\cref{fig:rolling-depth-age}). % too nonscientific?
% global patterns we observe
The youngest part of the record shows relatively low amplitude in the
...
that differences in the methods lead to different results! perhaps add new subheader sensitivity analysis or so.]}
...
Cohort project 3 \rez{and U.S. NSF grants
OCE20-01022, OCE20-34660 to R.E.Z.}
