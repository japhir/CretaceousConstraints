...
One of the key differentiating factors between different astronomical solutions is the timing of \acrpl{VLN}.
...
The long eccentricity cycles were closely matched to each solution, while the short eccentricity filter seems in general alignment with the solutions in those intervals where the solution has a high short eccentricity amplitude.
...
Rapid changes in sedimentation rate or lithology could have disrupted the bandpass filtering.
Therefore, it is more straightforward to focus on bundles with a high amplitude in the short eccentricity cycle in the data.
For example, the records show higher amplitude of short eccentricity in bundle \ma{5}, matching well with all astronomical solutions under evaluation here.
...
This could indicate that a \gls{VLN} was absent in this interval and that the solution was different from the astronomical forcing that the Earth experienced, likely due to chaos.

% should we talk about Ma405 bundle 2  that doesn't have high amplitude in the proxy records,
% but does have very high amplitude in the relief in the field/photographs
...
The highest-amplitude interval in the record occurs during bundle \ma{8} (\appr\qty{68.9}{\millionyearago}) and results from the darker MARLY? interval at around \qty{117}{\metre}.
This interval shows a strong excursion in both \gls{L*} and \gls{MS} (in the opposite direction) and closely matches the duration of one short eccentricity cycle.
% this weird (because it's so pointy and large) thing is soooo influential! I tried filtering it out too
It is referred to as ``escal\'{o}n'', and was previously associated with a short eccentricity maximum, while other causes such as regional factors or tectonic events could not be excluded~\cite{Dinares-Turell2013}.
