...
\item We use geological data from the \rez{Latest} Cretaceous to constrain the chaos of the solar system to understand Earth's orbital history.
\item \rez{Sections from Zumaia and Sopelana} are most compatible with solution ZB20a for the Maastrichtian.
\item This study \rez{provides} an astronomical time scale for applications in geology and paleoclimate.
% up to three
...
\rez{Intervals} with \acrfullpl{VLN} (low amplitude in the short eccentricity cycle) in the astronomical solutions that coincide with large amplitudes in the short eccentricity-related peaks in the filtered proxy record rule out alternatives.
...
Since the breakthroughs that proved the astronomical theory of the ice ages~\cite<i.e.,>[for review]{Emiliani1955,Hays1976,Hilgen2010},
\rez{ [ya, not good either way. need Hilgen10?]
\cite<i.e.,>{Emiliani1955,Hays1976}
\cite<see>[for review]{Hilgen2010}
}
...
\rez{The chaos manifests itself in different astronomical solutions
and is expressed, for example,} in the phasing of the different cycles.
...
Chaotic resonance transitions can change the timing of these \glspl{VLN} so that they occur with a period of \rez{about} \appr\qty{1.2}{\millionyear} instead~\cite{Laskar2011a,ZeebeLourens2019}.
...
Previous workers have attempted to put geological constraints on the chaos for the Paleocene and \rez{Eocene~\cite{ZeebeLourens2019,ZeebeLourens2022EPSL}
or identified most suitable existing solutions~\cite{Westerhold2017}}
...
In this work, we use the best available records immediately prior to the \acrfull{KT} (informal use) to constrain which astronomical solutions most accurately reflect Earth's orbital past.
...
\rez{The goal of the present study is to provide} an astronomical time scale for applications in geology and paleoclimate in the Maastrichtian.
...
For the present study, we selected the Zumaia and Sopelana I\rez{???} field sections,
...
The Zumaia section is one of the first 100 geological heritage sites, which was described as ``One of the best exposed, most continuous and highly studied outcrops of deep marine sediments in the world'' \cite[pp.~71--72]{Hilario2022} \rez{,
...
whose occurrence results in many random spectral peaks, but may also include an eccentricity and obliquity signal for cycles \ma{1--4}~\cite<the first four long eccentricity bundles prior to the \gls{KT} \rez{; Ma refers to Maastrichtian here},>{tenKateSprenger1993}. % this part is not mentioned in Batenburg
...
% which proxies?
\citeA{Batenburg2012} measured the \gls{MS}, the \gls{d13C}, and the \acrfull{L*} of rock samples (among others) from Zumaia, but we focus on \gls{L*} in the main manuscript \rez{(as explained below)}.
The \citeA{Batenburg2012} proxy record was extended downwards to include Sopelana I in \citeA{Batenburg2014}.
Unfortunately, the \ce{CaCO3} record from \citeA{tenKateSprenger1993} currently spans bundles \ma{1--4} \rez{[add reference to fig?]} and has not yet been extended to encompass the full Maastrichtian.
...
and
    \item Applied a piecewise linear fit based on depth intervals where the lithology changed \rez{substantially?} dramatically.
...
These filter parameters were selected after analyzing the sensitivity to different ways of filtering (rectangular, gaussian, different values for the taner filter roll parameter \rez{, see} \cref{fig:filter-windows,fig:full-RMSD-filter}).
The filtered proxy signals were then summed and scaled
\rez{(see above)}.
To improve legibility, we refer to the filtered summed scaled data as `filtered record' throughout the manuscript.
The scaled astronomical solutions were then \rez{subset? interpolated?} to the same timesteps as the data.
...
\end{equation}
where \(e\) is the scaled eccentricity of the astronomical solution and \(f\) is the scaled sum of long- and short eccentricity-related filters in the data (i.e., the filtered record) \rez{ ; $n$ is ...?}
...
Once the optimal (lowest \gls{RMSD}) tie-point depth was found, we fixed \rez{the depth} and moved on to the next-youngest tie-point.
...
\ijk{[Not sure if this paragraph is interesting enough to talk about.]}\rez{keep if you like}
...
Crucially, however, some astronomical solutions have a low amplitude in the short eccentricity component in intervals where the data \rez{shows} a relatively large amplitude.
...
In bundle \ma{5} (\appr\qty{67.8}{\millionyearago}) ZB20b has a relatively lower amplitude (with a \gls{VLN} in \ma{6}) than our filtered record, whereas ZB20c and ZB20d \rez{match} the data more closely and ZB20a matches it in amplitude but seems slightly out of phase.
...
During \ma{10} (\appr\qty{69.7}{\millionyearago}) the composite record also \rez{indicates} a large amplitude of short eccentricity, resulting in a mismatch with ZB20b and to a lesser extent La10c (where the \gls{VLN} seems to occur between \ma{9} and \ma{10}).
...
\begin{tablenotes}
    ...
    \item [b] \rez{Indicates whether} the astronomical solution \rez{lacks} a \acrfull{VLN} \rez{, consistent within} the interval where the filtered \gls{L*} record \rez{shows} large amplitude in the short eccentricity related cycle.
    \item [c] \Acrfull{L*}.
    \item [d] \Acrfull{d13C}.
    \item [e] \Acrfull{MS}.
    \item [f] Did not show a \gls{VLN} at around \qty{61}{\millionyearago},
    \rez{in contrast to the data}~\cite{ZeebeLourens2022EPSL}.
...
The cumulative \gls{RMSD} scores \rez{[rephrase: indicate when our filtered records diverged to what extent]} from the astronomical solutions (\cref{fig:cum-RMSD-all}).
...
\rez{Unsurprisingly,} the
\rez{cumulative or rolling?} \gls{RMSD} scores highlight that none of the astronomical solutions showed an amplitude that is as large as the filtered record in \ma{8}, resulting in a rapid increase of the cumulative \gls{RMSD} scores for all astronomical solutions under evaluation for this interval.
\rez{To match the \ma{8} data amplitude would require $e > 0.08$???,
whereas $\max{e} < 0.065$??? in all of our solutions.}
...
\ijk{[@REZ: what do you think of this section? It feels like it might belong in the discussion, but I \emph{am} talking about the descriptions/patterns as results].}\rez{[yap, discussion]}
...
\rez{The reason is that} even the most well-preserved astronomically forced signals from sediments do not reflect only astronomical forcing, but also a combination of the climate signal, the paleo-environment, the lithology that records the signal deposition, effects of diagenesis, \rez{the proxy used} to reconstruct the climate signal, and the parameters and choices of our analysis (see \ref{sec:sensitivity}).
Furthermore, the \rez{number of} slightly different valid astronomical solutions from small parameter variations is, due to the chaotic nature of the solar system, very large for the time interval at hand.
This means that a comparison against seven different calculations of astronomical forcing is not likely to represent an \rez{exact} match.
...
Therefore, the ZB20a solution is most compatible with the available Maastrichtian data \rez{and, at this point, is recommended} for tuning up to \appr\qty{71}{\millionyearago}
\rez{(see \cref{tab:results} for a ranking of the astronomical solutions according to these criteria)}.

Our sensitivity analysis (\ref{sec:sensitivity}) shows that the methodology is quite sensitive to parameter choices, however.
This may be the result of the proxy archive itself, which, while showing a clear preference for the ZB20a solution when \rez{considering} the \gls{L*} proxy, shows a slight preference for the ZB20d solution \rez{for} the \gls{MS} proxy (\cref{fig:full-RMSD-all}) and no clear differences \rez{for} the \gls{d13C} proxy.
\rez{However, as described above (see \cref{sec:data}), we argue that the} color reflectance \gls{L*} record most directly reflects the orbital forcing and is \rez{hence the preferred proxy archive for} inferences about the most accurate astronomical forcing scenario in this instance.
Different ways of pre-processing the data are discussed in \ref{sec:sensitivity}, and highlight the importance \rez{of} careful selection of a set of parameters that results in the best matches, while not violating constraints from other data (e.g., sedimentation rates, the age of the \gls{KT}, etc.).
...
One \rez{potential} caveat to this conclusion is that section photographs from the field site of Zumaia may show indications for a large amplitude of the short eccentricity related cycle in \ma{2}~\cite{Batenburg2012,Dinares-Turell2013}.
...
Future studies should aim to generate high resolution proxy records that have recorded the expression of both long and short eccentricity and connect to the oldest geologically constrained \rez{parts} (\appr\qty{71}{\millionyearago}) of the astronomical solutions.
...
We further suggest that future studies generate new astronomical solutions with similar initial settings to the ZB20a solution and, using appropriate parameter variations, explore \rez{the space beyond the horizon of predictability for a unique solar system solution.}
