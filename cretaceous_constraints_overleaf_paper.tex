...
Nevertheless, \acrfullpl{VLN} in the astronomical solutions that coincide with large amplitudes in
\rez{whereas}
the short eccentricity-related peaks in the filtered proxy record rule out alternatives, leaving only\rez{favoring} the ZB20a solution for cyclostratigraphy during the latest Cretaceous.
% decided to not focus on the \gls{KT} age for now.
\end{abstract}

\rez{remove full paragraph, see their table B.1}
\rez{KPG to KT}
...
the calculations of the astronomical solutions result in different outcomes with minor perturbations in initial conditions or parameter values beyond \appr\num{50} million years ago (\si{\millionyearago})~\cite<e.g.,>[]{Laskar2004,Laskar2011,Zeebe2017}. % what should I cite here? this is just quick 'n dirty.
...
is variable between different astronomical solutions due resonance transitions related to \(g_{4}\) and \(g_{3}\),
...
The long eccentricity cycle (\(g_{2}-g_{5}\), period of \qty{405}{\kiloyear}), on the other hand, is thought to be stable over much longer time scales.
%\ijk{[Do we need abbreviations for long and short eccentricity? Like \(e_{l}\) and \(e_{s}\) or SEC and LEC?]}
\rez{... not the case (ZL, AJ, 2024}
...
For example, the long eccentricity minimum near \qty{65.9}{\millionyearago} differs by up to \qty{70}{\kiloyear} between solutions La11~\cite{Laskar2011} and ZB20a~\cite{ZeebeLourens2022EPSL}.
...
One further key differentiating factor between different astronomical solutions is the timing of \glspl{VLN}.
When the individual components that make up the short eccentricity cycle interfere to cancel each other out, only the long eccentricity component remains clearly visible~\cite<see>[]{ZeebeLourens2022EPSL}.
Chaotic resonance transitions can change the timing of these \glspl{VLN}~\cite{ZeebeLourens2019}.
\ijk{[merge with general short ecc discussion b/c this is the g4-g3 cycle?]}
...
Previous workers have attempted to put geological constraints on the chaos for the Paleocene and Eocene~\cite<>[respectively]{ZeebeLourens2019,ZeebeLourens2022EPSL}.
...
In this work, we use the best available records immediately prior to the \gls{KT} (informal use)
\rez{inconsistent}
to constrain which astronomical solutions most accurately reflect Earth's orbit.
...
% which proxies?
\citeA{Batenburg2012} measured the \gls{MS}, the \gls{d13C}, and the \gls{L*} of rock samples (among others) from Zumaia, but we focus on \gls{L*} in the main manuscript.
...
\begin{tablenotes}
  ...
  \item [a] \(\Delta{}t\) = timestep, \(J_{2}\) = solar quadrupole moment, \(N_{\text{ast}}\) = N\textsuperscript{o} of asteroids, NLWP = N\textsuperscript{o} of lightweight particles.
...
In bundle \ma{5} (\appr\qty{67.8}{\millionyearago}) ZB20b has a relatively lower amplitude (with a \gls{VLN} in \ma{6}) than our filtered record, whereas ZB20c and ZB20d matches the data more closely and ZB20a matches it in amplitude but seems slightly out of phase.
...
The overall fits between our filtered data and the astronomical solutions (as measured by the \gls{RMSD}) were relatively similar between the different astronomical solutions.
...
\begin{table}
\begin{threeparttable}
\caption{\gls{RMSD}\tnote{a} score and compatibility with data for astronomical solutions\label{tab:results}}
\centering
...
La10c & 1.11 & 1.17 & 1.03 & Yes & Yes & No\tnote{*} & 6\tnote{f}\\
La10b & 1.09 & 1.18 & 1.09 & Yes & No\tnote{*} & Yes & 7\tnote{f}\\
\hline
\end{tabular}
\begin{tablenotes}
    \item %\gls{L*} \gls{RMSD} scores \(\ge1.1\) and
    \item [a] \Acrlong{RMSD} between normalized solution and normalized filtered record sum.
    \item [b] Whether the astronomical solution lacked a \acrfull{VLN} in the interval where the filtered data sum showed high amplitude in the short eccentricity related cycle.
    \item [c] \Acrfull{L*}.
    \item [d] \Acrfull{d13C}.
    \item [e] \Acrfull{MS}.
...
The cumulative \gls{RMSD} scores indicate when our filtered data diverged to what extent from the astronomical solutions (\cref{fig:cum-RMSD-all}).
...
The rolling \gls{RMSD} scores highlight that none of the astronomical solutions showed an amplitude that is as large as the filtered data in \ma{8}, resulting in a rapid increase of the cumulative \gls{RMSD} scores for all astronomical solutions under evaluation for this interval.
The rolling \gls{RMSD} score only increased slightly in \ma{10} for ZB20b, whereas there appears a clear coincidence of a \gls{VLN} in the solution paired with a relatively high amplitude in the eccentricity construct.
...
Furthermore, the potential for generating slightly different valid astronomical solutions from small parameter variations is, due to the chaotic nature of the solar system, is very large for the time interval at hand.
...
However, both qualitatively (with the visual comparison of the \glspl{VLN}) and quantitatively (with the (rolling) \gls{RMSD}\rez{squiggly lined}\ijk{but I don't know what for}), we can distinguish which of the solutions best matches the astronomical forcing that is detectable in the proxy archive.
...
After careful consideration of these factors, we argue that it is possible to select an optimal set of parameter and filtering frequencies/widths that results in a reliable outcome to the extent that the data sets allow for.
...
The bootstrapped scores indicate that the differences between filtered data based on the \gls{d13C} proxy records for the different astronomical solutions were likely not significant,
...
% shift
There are upper bounds to the extent by which we can let the parameters of our matching algorithm fluctuate, however.
...
The same type of shift occurred when we changed the width of the filters (not shown).
