...
\item Geological data can be used to constrain the chaos of Earth's orbital past. \rez{it's not really Earth's orbit that's chaotic but the solar system}
\item The Zumaia and Sopelana terrestrial sections are most compatible with solution ZB20a for the Maastrichtian. \rez{terrestrial? i thought hemipelagic? deposition matters,
...
%Problem
Astronomical solutions form the backbone of \rez{drop: precise and} accurate dating for geology and paleoclimate studies.
...
Geological data have been used to constrain this chaos in order to arrive at a geologic time scale up to the \rez{end-Cretaceous} \acrlong{KT}.
%Action
...
%Results
We find that the filtered \gls{L*} \rez{$L^*$ ?} record is most compatible with \rez{the} astronomical solution ZB20a.
However, these results are sensitive to parameter choices in our algorithm, which we describe in detail.
Nevertheless, \gls{VLN} in the astronomical solutions that coincide with very high \rez{large?} amplitudes in the short eccentricity-related peaks in the filtered proxy record give a clear indication on why the ZB20a solution should be favored when performing cyclostratigraphy for the latest Cretaceous.
...
In this work, we use geological data to constrain astronomical solutions that most accurately reflect \rez{Earth's orbit in order to ultimately provide} an astronomical time scale for applications in geology and paleoclimate.
For previous on the Paleocene and Eocene, see \citeA{Westerhold2017,ZeebeLourens2019,ZeebeLourens2022EPSL}.
The Walvis Ridge records \rez{which?} are exceptional in their quality, so finding comparable records that capture precession-scale alternations and patterns of amplitude modulation for older time periods is difficult at this time.
...
One of the key differentiating factors between different astronomical solutions is the timing of \gls{VLN} \rez{VLNs}.
When the individual components that make up the short eccentricity cycle (i.e., \appr\qtylist{95;99.7;124;132.5}{\kiloyear}) \rez{should be ca. 95,99,124,131} interfere to cancel each \rez{other} outher out, only the long eccentricity component (\appr\qty{405}{\kiloyear}) remains clearly visible.
...
we have considered several continental sections \rez{are they depositional-continental?} of marine successions and sea floor drill core sites.
...
The \gls{d13C} proxy showed \rez{clear long eccentricity-related cycles} but the short eccentricity was less clearly expressed,
...
The age model for the Zumaia and Sopelana sites was based on the identification of long eccentricity minima in the field \rez{by} \citeA{Batenburg2012,Batenburg2014}.
...
A previous best-estimate of the age of the \rez{KTB} \gls{KT} \cite<\qty{65.9}{\millionyearago},>[]{ZeebeLourens2022EPSL} anchored the floating age model to absolute time.
...
This has previously been suggested to potentially indicate a \qty{1.2}{\millionyear} cycle~\citeA{Batenburg2014} \rez{wrong citation "()", check throughout ms}.
...
We \rez{[too many We's]} have tried many ways of detrending the records, for example with larger and smaller chunks in the depth domain (\cref{fig:detrend}), or with different frequencies for the lowpass filter, and report sensitivity to this in the appendix.
...
\end{equation}
where \(e\) is the scaled eccentricity of the astronomical solution and \(f\) is the scaled and filtered combination of long- and short eccentricity \rez{based on the data?}.
...
This could be related to the gradual transition that we see in the \gls{L*} data towards the base of the \gls{KT} or to edge effects related the bandpass filtering.
...
\rez{However, this[what?] was different from the visual indications based on the relief and contrast from the section photographs in \citeA<>[Fig. 2]{Batenburg2012} and \citeA<>[Fig. 5a]{Dinares-Turell2013}}
...
It is referred to as ``escal\'{o}n'' \rez{(step?)}, and was previously also associated with a short eccentricity maximum, while other causes such as regional factors or tectonic events could not be excluded~\cite{Dinares-Turell2013}.
...
Our results were relatively sensitive to the choices of \rez{the} proxy archive we used, how we detrended the record to subtract non-cyclical patterns, and how we performed bandpass filtering.
% proxy
...
% boot
One approach to \rez{obtain} get a sense of the uncertainty of our \gls{RMSD} scores is to perform bootstrapping.
...
These bootstrapped scores indicate that the differences between eccentricity constructs based on the \gls{d13C} proxy records for the different astronomical solutions were likely not significant, while the lower score for the \gls{L*} ZB20a solution likely was.
...
% tiepoint tweaks
Furthermore, allowing each \rez{tie point} tiepoint depth to vary by larger distance than our default value of \qty{1.6}{\metre} usually results in slightly better \gls{RMSD} scores (\cref{fig:full-RMSD-tie-error}).
...
Ultimately, this will allows us to anchor events in Earth's geologic and paleoclimate history and to distinguish causes from effects in the climate system.
...
This may be the result of the proxy archive itself, which, while showing a clear preference for the ZB20a solution, is not able to robustly distinguish between astronomical solutions across the Maastrichtian.
...
Future studies could aim to generate high resolution proxy records that have recorded the expression of both long and short eccentricity and connect to the oldest geologically constrained parts of the astronomical solutions.
It may be wise to target low latitudes for this, since eccentricity-related cyclicity tends to dominate in the tropics~\cite<Fig. 7a>[]{LaeppleLohmann2009}. % during the late Pleistocene (750 ka to present) in model
