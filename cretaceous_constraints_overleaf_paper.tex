...
% REZ: i'm struggling with the order in which to put this, first all the details of sensitivity analysis and then main results, so that conclusions naturally flow from the results or first main findings then nitty-gritty stuff (but then we're not able to describe the detrend types and such as nicely)
...
% The \gls{RMSD} scores for each of the astronomical solutions for the full record of the Zumaia and Sopelana sites were lowest for different solutions based on different ways of detrending the record (\cref{tab:detrend_types}). \ijk{that's too long and repetitive.}
