...
%Results
We find that the filtered sum \gls{L*} record is most compatible with the astronomical solution ZB20a.
However, these results are sensitive to parameter choices in our algorithm, which we describe in detail.
Nevertheless, we present evidence in favor of solution ZB20a for cyclostratigraphy during the latest Cretaceous.
Periods with \acrfullpl{VLN} (low amplitude in the short eccentricity cycle) in the astronomical solutions that coincide with large amplitudes in the short eccentricity-related peaks in the filtered sum proxy record rule out alternatives.
...
Furthermore, the occurrence of \acrfullpl{VLN} can be ruled out when the data show a high amplitude in the short eccentricity cycle.
...
\begin{figure}
  ...
  \caption{\label{fig:rolling-depth-age}
    \textbf{Maastrichtian Zumaia (purple) and Sopelana (orange) inverted filtered summed normalized \gls{L*} records\ijk{TODO: might need to be rephrased} tuned to astronomical solutions (black, first 7 panels).}
...
The records show a higher amplitude of the short eccentricity-related filter in bundle \ma{5}, matching well with all astronomical solutions under evaluation here (\cref{tab:results}).
...
% Note that calculating correlation coefficients between the filtered record sum and the astronomical solutions shows the same patterns (calculated using \texttt{astrochron::surrogateCor()} for a Pearson, Spearman, and Kendall correlation, not shown).
...
\begin{tablenotes}
    ...
    \item [b] Whether the astronomical solution lacked a \acrfull{VLN} in the interval where the filtered sum of the \gls{L*} record showed high amplitude in the short eccentricity related cycle.
    \item [c] \Acrfull{L*}.
    \item [d] \Acrfull{d13C}.
    \item [e] \Acrfull{MS}.
...
\begin{figure}
    ...
    \caption{\label{fig:full-RMSD-all} % all being all proxies
      \textbf{Best overall matches.}
        \Acrfull{RMSD} scores (lower is better) of compilation of Zumaia and Sopelana filtered summed proxy records against several orbital solutions.
...
The rolling \gls{RMSD} score only increased slightly in \ma{10} for ZB20b, whereas there appears a clear coincidence of a \gls{VLN} in the solution paired with a relatively high amplitude in the filtered summed record.
...
\ijk{not done yet but I like the above}
The \gls{MS} record from ODP Site 1267B, spanning \ma{1--6.5}~\cite<Fig. 4,>[]{Husson2011}
%but note that they did not use the splice table from~\cite{Westerhold2008} to convert from MBSF to RMCD.
...
Allowing each tie point depth to vary by larger distance than our default value of \qty{1.6}{\metre} (about \qty{10}{\percent} of the long eccentricity cycle) usually results in slightly better \gls{RMSD} scores (\cref{fig:full-RMSD-tie-error}).
