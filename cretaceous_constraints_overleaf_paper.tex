...
\begin{keypoints}
%\item The best available records from the late Maastrichtian do not help
%constrain Astronomical Solutions. ?? Too long, can do better.
...
the Paleocene and Eocene, see \citeA{ZeebeLourens2019,ZeebeLourens2022EPSL}.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.9\linewidth]{Zumaia-Sopelana_highpass.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{Zumaia-Sopelana_lowpass_spectra.png}
...
\end{figure}
% \texttt{value} Use the proxy value as reported in the original study. This
% does not exclude linear interpolation, demeaning (subtracting the mean) and
% detrending (subtracting a linear fit for the whole record) for certain
% spectral analysis tools.

% \texttt{scl} Subtracting the mean and dividing by the standard deviation
% results in a normalized or scaled value.

% \texttt{gam\_det} A gam is a generalized additive model with integrated
% smoothness estimation. We fit this smooth curve and then subtract it from the
% original proxy value.

% \texttt{lin\_scl} We fit a linear model for each of the red, white, and
% orange intervals separately. Then we subtract these models from the raw proxy
% value to get a detrended result.
...
\begin{figure}[htb]
  \centering \includegraphics[width=1.2\textwidth]{rolling_improvement.png}
...
study are from \citeA{Batenburg2012} and \citeA{Batenburg2012}.

Analysis was performed using the R programming language \cite{RCoreTeam2020}
and made use of \texttt{astrochron} \citeA{Meyers2014} and the
\texttt{tidyverse} \citeA{Wickham2019}. The new R package
\texttt{AstronomicalSolutions} will be made available on publication on
\url{https://github.com/japhir/AstronomicalSolutions} \citeA{Kocken2024}.
