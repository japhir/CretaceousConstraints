...
\rez{[goal: astro time scale FOR geo apps]}
for previous work on the Paleocene and Eocene, see \citeA{Westerhold2017,ZeebeLourens2019,ZeebeLourens2022EPSL}.
...
Here we aim to use the best available records older
\rez{[all older records? need to define geol interval]}
than the \gls{KT} boundary (informal use) to constrain which astronomical solution best reflects this time period.
...
\rez{[For the present study, we selected ..., which]}
We think that the Zumaia and Sopelana field sections show the clearest orbitally-forced patterns for the Maastrichtian,
...
and comprises rhythmically-bedded hemipelagic limestone--marl alternations~\cite{tenKateSprenger1993,Pujalte1999}. % this is cited as Pujalte 1998 in Batenburg2012, year off-by-one!
See references in \citeA{Batenburg2014} for some historical context of the study sites. % this has better refs than their 2012 paper I think.
...
the original refs]}
The upper section of Zumaia still contains many thin turbidites~\cite{tenKateSprenger1993},
but they do not disrupt the pattern of limestone--marl alternations in the upper Maastrichtian~\cite{Batenburg2012}.
The alternations in color and bed resistance of the smallest-scale alternations of \appr\qty{70}{\cm} were logged in detail~\cite{Batenburg2012}
and have been associated with climatic precession forcing (\appr\qty{23}{\kiloyear}),
while the larger bundling patterns represent short (\appr\qty{4}{\metre}, \appr\qty{100}{\kiloyear})
and long (\appr\qty{16}{\metre}, \appr\qty{405}{\kiloyear}) eccentricity amplitude modulation~\cite{Batenburg2012,Batenburg2014}.
Precession minima correspond to enhanced seasonal contrast in the Northern hemisphere during eccentricity maxima,
resulting in deposition of thicker darker marls due to an enhanced hydrological cycle~\cite{Batenburg2014}.
...
For \gls{MS} it is unclear how it??? relates to the climate forcing and the original study reports that the values show scatter,
...
Here we assume \rez{independent durations?} of \qty{405}{\kiloyear} for each of these long eccentricity cycles to arrive at an initial floating age model.
...
and 3. We apply a piecewise linear fit based on depth intervals where the lithology changes dramatically, or we observe long-term trends that are at a larger scale than long eccentricity.
...
The detrended data (\cref{sec:detrend}) is combined with the initial age model (\cref{sec:agemodel}) to \rez{obtain} data in the time domain.
...
We chose these parameters after looking??? at sensitivity to different ways of filtering (rectangular, gaussian, different parameters for the taner filter in \cref{fig:full-RMSD-filter,fig:filter-windows}).
