...
The long eccentricity cycles are closely matched to each solution, while the short eccentricity filter seems in general alignment with the solutions in the intervals where the solution has a high short eccentricity amplitude.
...
For example, the records show higher amplitude in short eccentricity in bundle \ma{5}, matching well with all astronomical solutions under evaluation here.
Crucially, however, some astronomical solutions have a low amplitude in the short eccentricity component in intervals where the data show a relatively high amplitude.

% not sure if this is interesting enough to talk about
...
%For the next long eccentricity bundle, the data have a low amplitude and so do most astronomical solutions.

% I should introduce VLNs towards the end of the intro probably
One of the key differentiating factors between astronomical solutions is the timing of \acrpl{VLN}.
...
For example, there may be changes in sedimentation rate or lithology that result in a low amplitude in the filtered short eccentricity cycle.
Therefore, it is more straightforward to detect bundles with a high amplitude in the short eccentricity cycle in the data, which may indicate that a \gls{VLN} was likely absent in this interval.

For example, in bundle \ma{5} (\appr\qty{67.8}{\millionyearago}) ZB20b had a relatively lower amplitude (with a \gls{VLN} in \ma{6}) than our eccentricity construct, whereas ZB20c and ZB20d matched the data more closely and ZB20a matched it in amplitude but seems slightly out of phase.
The highest-amplitude interval in the record occurs during bundle \ma{8} (\appr\qty{68.9}{\millionyearago}) and results from the darker interval at around \qty{117}{\metre}.
This interval shows a strong excursion in both \gls{L*} and \gls{MS} (in the opposite direction) and closely matches the duration of a short eccentricity cycle.
% this weird (because it's so pointy and large) thing is soooo influential! I tried filtering it out too
It is referred to as ``escal\'{o}n''~\cite{Dinares-Turell2013}.
...
This is significant, because solutions ZB18a, and ZB20c have a low amplitude of short eccentricity in this interval.
...
Furthermore, changing the parameters of the taner filter has an effect: when using a more rectangular window (roll of \num{e10} or a smoother window (roll of \num{e3}) results in different ``best'' solutions (\cref{fig:full-RMSD-filter,fig:filter-windows}).
