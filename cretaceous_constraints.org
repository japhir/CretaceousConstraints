...
#+begin_example
----- FINDING MAXIMA OF PEAKS, FILTERING AT THRESHOLD VALUE -----
 ,* Number of data points= 249001
 ,* Number of columns= 2
 ,* Identifying maxima of peaks
 ,* Number of peaks identified= 129
...
----- FINDING MAXIMA OF PEAKS, FILTERING AT THRESHOLD VALUE -----
 ,* Number of data points= 187500
 ,* Number of columns= 2
 ,* Identifying maxima of peaks
 ,* Number of peaks identified= 152
...
----- FINDING MAXIMA OF PEAKS, FILTERING AT THRESHOLD VALUE -----
 ...
 ,* Number of columns= 2
 ,* Identifying maxima of peaks
 ,* Number of peaks identified= 148
...
----- FINDING MAXIMA OF PEAKS, FILTERING AT THRESHOLD VALUE -----
 ...
 ,* Number of columns= 2
 ,* Identifying maxima of peaks
 ,* Number of peaks identified= 150
...
----- FINDING MAXIMA OF PEAKS, FILTERING AT THRESHOLD VALUE -----
 ...
 ,* Number of columns= 2
 ,* Identifying maxima of peaks
 ,* Number of peaks identified= 158
...
----- FINDING MAXIMA OF PEAKS, FILTERING AT THRESHOLD VALUE -----
 ...
 ,* Number of columns= 2
 ,* Identifying maxima of peaks
 ,* Number of peaks identified= 155
...
:END:
#+begin_src R :results output graphics file :file imgs/rolling_age_overview_best.pdf :width 5 :height 5
  best_age_optim_one |>
    filter(comb == "1:1") |>
    ## filter(detrend_type != "value") |>
    ## filter(detrend_type == "lin_scl_med") |>
    ...
    ## geom_point(aes(y = RMSD_start), size = 0.7) +
    ## geom_path(aes(y = RMSD_start, linetype = "Field")) +
    geom_line(aes(x = sol, y = RMSD, group = "all"), colour = "gray")  +
    geom_point(size = 1)
    ## geom_segment(arrow=arrow(type = "closed", length = unit(0.1, "inches")))+
    ## geom_linerange(aes(ymin = RMSD_start, ymax = RMSD),
    ##                alpha = .2) ## +
...
#+begin_src R :results output graphics file :file imgs/full_RMSD_scores.pdf :width 8 :height 5
  ...
  best_age_optim |>
    ...
    ## ggplot(aes(x = age * 1e-3, y = ecc, colour = site)) +
    ## facet_grid(rows = vars(sol)) +
    ## scale_x_reverse() +
    ## geom_line(aes(group = paste(site, sol, detrend_type, window, comb)))
    summarize(RMSD = sqrt(mean(SD)),
...
#+begin_src R
  smooth_optim_age_value <- best_age_optim_one |>
...
#+RESULTS:
: filter: removed 99 rows (79%), 26 rows remaining
  ...
  specs_optim_age_pl <- smooth_optim_age_value |>
    ...
    mutate(mtm = map(data,
        ...
        dplyr::rename(frequency = .data$Frequency,
                      power = .data$Power,
                      harmonic_cl = .data$Harmonic_CL,
                      powerlaw_cl = .data$PowerLaw_CL,
                      powerlaw_fit = .data$PowerLaw_fit)
                     )) |>
    select(-data) |>
    unnest(cols = c(mtm))
#+end_src
...
#+begin_src R :results output graphics file :file imgs/Zumaia_Sopelana_spectra_optimal.png :width 2400 :height 2200 :res 300
  ...
  specs_optim_age |>
    ...
    ## annotation_logticks(sides = "l") +
    ## # plot AR1 fits/confidence levels
    ## geom_line(aes(y = ar1_power,
    ##               group = paste(site, detrend_type, sol, .width))) +
    ## geom_line(aes(y = Harmonic_CL)) +
    geom_ribbon(aes(ymin = ar1_fit, ymax = ar1_power,
                    linetype = NA, fill = "AR1",#fill = detrend_type,
                    group = paste(detrend_type, site, sol, proxy, .width)),
                  alpha = .1) +
    geom_ribbon(aes(ymin = ar1_fit, ymax = ar1_power,
                    linetype = NA, fill = "AR1",#fill = detrend_type,
                    group = paste(detrend_type, site, sol, proxy, .width)),
                alpha = .3,
                data = specs_optim_age_tap_10 |> my_filter()) +
    geom_ribbon(aes(ymin = powerlaw_fit, ymax = powerlaw_power,
                    linetype = NA, fill = "PL",#fill = detrend_type,
                    group = paste(detrend_type, site, sol, proxy, .width)),
                alpha = .1,
                data = specs_optim_age_pl |>
                  my_filter()) +
    geom_ribbon(aes(ymin = ar1_fit, ymax = ar1_power,
                    linetype = NA, fill = "ML96",#fill = detrend_type,
                    group = paste(detrend_type, site, sol, proxy, .width)),
                alpha = .1,
                data = specs_optim_age_ML96 |>
                  my_filter()) +
    geom_ribbon(aes(ymin = lowspec_fit, ymax = lowspec_power,
                    linetype = NA, fill = "LOWSPEC",
                    group = paste(detrend_type, site, sol, proxy, .width)),
                alpha = .1,
    ...
    ##           data = specs_optim_age_tap_10 |> my_filter()) +
    # same as above
    ## geom_line(aes(colour = "MTMML96",
    ##               group = paste(site, sol, proxy, detrend_type)),
    ##           data = specs_optim_age_ML96 |> my_filter()) +
    geom_line(aes(y = Median_Smoothed_Power, colour = "MTM ML96",
                  group = paste(site, sol, proxy, detrend_type)),
              data = specs_optim_age_ML96 |> my_filter()) +
    ## geom_line(aes(x = Frequency, y = `MTM/AR`, colour = "MTM AR",
    ##               group = paste(site, sol, proxy, detrend_type)),
    ##           data = specs_optim_age_AR |> my_filter()) +
    # same as above
    ## geom_line(aes(colour = "PL",
    ##               group = paste(site, sol, proxy, detrend_type)),
    ##           data = specs_optim_age_pl |> my_filter()) +
    geom_line(aes(colour = "LOWSPEC",
                  group = paste(site, sol, proxy, detrend_type)),
