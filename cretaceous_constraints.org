  ...
  best_age_optim <- rolling_age_optim |>
    ...
    mutate(RMSD = map_dbl(the_best,
                          \(x) x$summary |>
                               ...
                               slice(1) |>
                               pull(RMSD_cum))) |>
    tidylog::filter(RMSD == min(RMSD), .by = c(sol, site, proxy, detrend_type, depth_chunk, comb, window))
#+end_src

#+RESULTS:
: filter: removed 7 rows (1%), 609 rows remaining
...
#+begin_src R :results output graphics file :file imgs/Zumaia_Lstar_1-1_solutions_simple.pdf :width 9.6 :height 8
  best_age_optim |>
    ...
    geom_text(aes(x = age * 1e-3, y = 4, #y = 4 + as.integer(depth_chunk),
                  label = round(RMSD, 3)),
              ...
              data = best_age_optim_one |>
                ...
                mutate(age = map_dbl(the_best,
                                     \(x) mean(x$matched$age,
                                                 na.rm = TRUE)))) +
    geom_text(aes(x = age * 1e-3, y = 5,# + as.integer(depth_chunk),
                  label = paste(site, depth_chunk, comb, detrend_type, window)),
              size = 2, show.legend = FALSE,
              data = best_age_optim_one |>
                my_filt() |>
                mutate(age = map_dbl(the_best,
