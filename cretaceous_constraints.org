  ...
  rolling_age_optim_old <-
    ...
    bind_rows(
      bind_rows(
        ...
        read_rds("out/2024-03-28_rolling_age_optim_Lstar_lin-scl-med_gauss.rds") |> mutate(frac = 0.3 * 0.5 * gauss_corr), # I think this was accidentally at 0.5x because I had my logic reversed
        read_rds("out/2024-03-28_rolling_age_optim_Lstar_lin-scl-med_gauss_2x.rds") |> mutate(frac = 0.3 * 2 * gauss_corr), # this was for sure at 2x
        read_rds("out/2024-03-28_rolling_age_optim_Lstar_lin-scl-med_gauss_3x.rds") |> mutate(frac = 0.3 * 3 * gauss_corr), # I think 3x is the key!
        read_rds("out/2024-03-28_rolling_age_optim_Lstar_lin-scl-med_gauss_narrow-0.25_3x.rds") |> mutate(frac = 0.25 * 3 * gauss_corr), # still 3x
        read_rds("out/2024-03-28_rolling_age_optim_d13C-MS_lin-scl-med_gauss_narrow-0.25_3x.rds") |> mutate(frac = 0.25 * 3 * gauss_corr), # still 3x
        read_rds("out/2024-05-06_rolling_age_optim_Lstar_lin-scl-med_gauss_0.25_tie-error-1.6_slide-300.rds") |> mutate(frac = 0.25 * 1.5 * gauss_corr), # scaled by 1.5x
        read_rds("out/2024-05-06_rolling_age_optim_d13C-MS_lin-scl-med_gauss_0.25_tie-error-1.6_slide-300.rds") |> mutate(frac = 0.25 * 1.5 * gauss_corr), # 1.5x
        read_rds("out/2024-05-07_rolling_age_optim_Lstar-d13C-MS_lin-scl-med_gauss_0.25_tie-error-1.6_slide-300.rds") |> mutate(frac = 0.25 * 2.15 * gauss_corr), # 2.15x
        read_rds("out/2024-05-07_rolling_age_optim_Lstar-d13C-MS_lin-scl-med_gauss_0.25_tie-error-1.6_slide-300_tweak.rds") |> mutate(frac = 0.25 * 2.1227 * gauss_corr), # 2.1227x (new value found after playing around with sim)
      ) |>
...
#+begin_src R :results none
  rolling_age_optim <-
    ...
    bind_rows(
      ...
      read_rds("out/2024-04-08_rolling_age_optim_d13C_MS_Lstar_lin-scl-med_taner_narrow-0.25_all-comb.rds") |> mutate(roll = 1e10, frac = 0.25),
    ) |>
...
#+RESULTS:
: filter: removed 136 rows (23%), 466 rows remaining
...
: filter: removed 4,972 rows (1%), 416,672 rows remaining
: filter: removed 273 rows (93%), 21 rows remaining
...
#+begin_src R
  ...
  overall_sd_old <- rolling_age_optim_old |>
    ...
    tidylog::filter(proxy == "d13C" | site == "Sopelana" |
                    (site == "Zumaia" & age < max_age_sopelana))
  rm(rolling_age_optim_old) # less chance of running out of memory
...
: filter: removed 25,904 rows (5%), 543,469 rows remaining
...
#+begin_src R :results output graphics file :file imgs/full_RMSD_roll_comparison.pdf :width 3.3 :height 2
  pl_rmsd_taner <- overall_best |>
    ...
    geom_line(aes(group = "all"), colour = "darkred",
              data = overall_best_old |>
                filter(detrend_type == "lin_scl_med", proxy == "Lstar", window == "gaussian", near(frac, 0.25 * 2.15 * gauss_corr, 0.001))) +
...
**** simplified comparison of windows: rect vs. gauss vs. taner
:PROPERTIES:
...
#+begin_src R :results output graphics file :file imgs/full_RMSD_filter_comparison.png :width 2600 :height 700 :res 300
  pl_rmsd_detrend_types <- overall_best |>
    ...
    ##             filter(detrend_type == "lin_scl_med",
    ##                    window == "gaussian",
    ##                    near(frac, 0.25 * 2.15 * gauss_corr, 0.001))) +
    ## # I think in the paper I don't want to show the narrow ones too here
    ## geom_point(aes(shape = window), colour = "darkred",
    ##            data = overall_best_old |>
    ##              filter(detrend_type == "lin_scl_med",
    ...
    theme(legend.position.inside = c(.1, .20),
          legend.position = "inside",
          legend.background = element_blank(),
          legend.key.height = unit(1, "mm")) #+
...
#+begin_src R :tangle /tmp/reprex.R
  ...
  simple <- tibble::tibble(age_floating = seq_len(N) - 1,
                           value = astrochron::cycles(freqs = c(1 / pl, 1 / ps), start = 0, end = N - 1, genplot = FALSE)$value,
                           noise = rnorm(N),
                           res = value + noise)
...
#+begin_src R :results output graphics file :file imgs/filter_windows.pdf :width 5 :height 3 :tangle /tmp/reprex.R
  ## mfa <- my_filt_age[1:2, ] |>
  ##   mutate(range = 0.25 * f, flow = f - range, fhigh = f + range) |>
  ##   mutate(window = "rectangular") |>
  ##   pivot_longer(flow:fhigh)
  ...
  pl_windows <- windows |>
    ...
    ##           colour = NA,
    ##           position = "identity",
    ##           data = slns_specs |> filter(frequency < 0.02) |> mutate(window = "sol"))
...
#+begin_src R :tangle /tmp/reprex.R
  ...
  # plot diffs
  bind_cols(to |> rename(to = value), go |> select(go = value)) |>
    as_tibble() |>
    mutate(diff = to - go) |>
    ggplot(aes(x = age_floating, y = diff)) +
    geom_line() +
    geom_line(aes(y = to, colour = "to")) +
    geom_line(aes(y = go, colour = "go"))
#+end_src
