...
#+begin_src R :results output graphics file :file imgs/new_rolling_improvement.png :width 2400 :height 1700 :res 300
  ...
  best_age_optim |>
    ...
    geom_point(fill = "black", shape = 25, #size = 2,
               position = my_dodge) +
...
#+begin_src R :results output graphics file :file imgs/Zumaia_Lstar_1-1_solutions.png :width 2400 :height 2000 :res 300
  best_age_optim |>
    ...
    geom_line(aes(y = ecc_sln), colour = "gold3", linewidth = 1) +
    geom_line() +
    geom_text(aes(x = 65.8, y = 0, label = round(RMSD, 3)),
              data = \(x) distinct(x, RMSD, sol))
#+end_src
