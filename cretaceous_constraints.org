...
**** NEXT gam removal with specified knots
:PROPERTIES:
:CREATED:  [2023-09-25 Mon 15:42]
:END:
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_records.png :width 900
...
*** create eccentricity curve from data
:PROPERTIES:
:CREATED:  [2023-09-26 Tue 16:51]
:END:
#+begin_src R
  ...
  smooth |>
    ## mutate(det = scale(value)[, 1], .by = c(site, name)) |>
    ggplot(aes(x = 66 + age / 1e3, y = value, colour = site, group = site)) +
    facet_grid(rows = vars(name), scales = "free_y") +
    geom_vline(aes(xintercept = 66 + age / 1e3), data = agem_simple) +
    ...
    ## geom_line(aes(y = envelope + 5.5, group = site), colour = "red", data = hb_age_prec) +
    scale_x_continuous(breaks = 66 + seq(0, 5, 1), labels = seq(0, 5, 1)) +
    labs(x = "K/Pg + Age (Ma)", y = "Normalized filtered proxy",
         ## title = "Zumaia raw proxy record filters"
         ## title = "Zumaia fancy detrended proxy record filters"
         )
#+end_src
...
#+begin_src R
  data_ecc <- filts_age |>
    ...
    mutate(ZB18a = approx(sln$age, sln$scl, xout = age + 66000)$y)
...
#+begin_src R
  ...
  data_ecc <- filts_age |>
    ...
    mutate(age_kpg = list(66000 + seq(-5000, 5000, 5))) |>
    unnest(age_kpg) |>
    unnest(data) |>
  ...
  data_ecc |>
    ...
    ggplot(aes(x = age_kpg, y = msd_1_2_env, group = site)) +
    geom_line() +
    geom_line(aes(y = msd_1_1_env)) +
    geom_line(aes(y = msd_1_0.5_env)) +
    geom_line(aes(y = msd_1_1)) +
    geom_line(aes(y = msd_1_1.5)) +
    geom_line(aes(y = msd_1_0.5)) +
    facet_grid(rows = vars(name)) +
