...
#+RESULTS:
: filter: removed 108 rows (18%), 494 rows remaining
...
#+RESULTS:
: Joining with `by = join_by(proxy, detrend_type, depth_chunk, comb, sol, date, frac, window)`
: filter: removed 4,957 rows (1%), 416,687 rows remaining
