...
This while the 405 kyr target filter range (\pm40 %) ranges from 0.0375 to 0.0875 cycles/m.
...
#+begin_src R
  smooth_hp <- comp |>
    ...
    mutate(
      ...
      notch = map(linterp,
                  ~ . |>
                    astrochron::lowpass(fcut = 0.035, xmax = 0.1, verbose = FALSE, genplot = FALSE) |>
...
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_highpass.png :width 2400 :height 1700 :res 300
  smooth_hp |>
    ggplot(aes(x = depth_linterp, y = value_linterp)) +
    facet_grid(cols = vars(site), rows = vars(proxy), scales = "free") +
    scale_x_reverse() +
    geom_line() +
    geom_line(aes(y = lowpass), colour = "red") #+
  ## geom_line(aes(y = highpass), colour = "purple")
