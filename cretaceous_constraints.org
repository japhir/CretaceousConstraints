...
**** SOME CaCO3: ten Kate and Sprenger 1993
:PROPERTIES:
...
#+begin_src R
  ...
  Zumaia_CaCO3 |>
    ...
    ## geom_line(aes(y = lin_pred), colour = "purple", linewidth = 2) +
    geom_line() +
  ...
  Zumaia_CaCO3 |>
    spectral_analysis(x = age, y = lin_scl) |>
    # they say analyisis in the depth domain doesn't make sense due to changing
    # sedrates
    plot_spectrum(ar1 = TRUE) +
  ...
  # this errors for some reason when linterping!
  Zumaia_CaCO3 |>
    bandpass_filter(frequencies = my_filt_age |> filter(target == "405 kyr"),
                    x = age, y = lin_scl,
                    add_depth = TRUE)
#+end_src
*** Age Model
...
src_R{agem_sol[agem_sol$sol=="ZB18a","depth_diff"]}
...
src_R{1/(agem_sol[agem_sol$sol=="ZB18a","depth_diff"] / 405 * 2000)}
...
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_records_complicated.png :height 1200 :width 2600 :res 300
  ...
  smooth |>
    ...
    geom_rect(aes(fill = str_sub(colour, 1, -2),
                  ...
                  ymin = max(value) + 2, ymax = max(value) + 4)) +

    # "raw" data
    geom_line(colour = "gray") +
    annotate("text", x = xnot, y = mean(value),
             label = "value", size = 2, hjust = 1) +
    # raw value scaled
    geom_line(aes(y = scale(value)[, 1]), colour = "gray") + # scaled
    annotate("text", x = xnot, y = 0, label = "scl = scale(value)",
             size = 2, hjust = 1) +
    ...
    annotate("text", x = xnot, y = -10, label = "gam_scl = scale(value-gam)",
             size = 2, hjust = 1) +
    # scale(value - gam_pred - lin_gam) = det
    ## geom_line(aes(y = det - 15), colour = "gray") +
    ## annotate("text", x = xnot, y = -15, label = "lin_gam_det = scale(value-gam-lin_gam)",
    ##          size = 2, hjust = 1) +
...
#+begin_src R :results output graphics file :file imgs/new_rolling_improvement.png :width 3400 :height 1700 :res 300
  ...
  best_age_optim |>
    ...
    labs(x = "Astronomical solution", shape = "Age model", linetype = "Age model") +
    ...
    geom_point(aes(y = RMSD_start, shape = "Field")) +
    geom_point(aes(fill = detrend_type, shape = "Optimized")) +
    geom_line(aes(y = RMSD_start, linetype = "Field")) +
    geom_line(aes(x = sol, y = RMSD, linetype = "Optimized")) +
...
#+RESULTS:
[[file:imgs/Zumaia_Lstar_1-1_solutions_simple.png]]
