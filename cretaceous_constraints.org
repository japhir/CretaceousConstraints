...
#+begin_src R
  ...
  freq_perc <- 0.4
  my_filters <- tribble(
    ~ target,  ~site,     ~ depth_chunk, ~ p,
    "405 kyr", "Zumaia", "full record", 16,
    "405 kyr", "Zumaia", "depth < 109.26", 16,
    "405 kyr", "Zumaia", "depth >= 109.26", 12,
    "405 kyr", "Sopelana", "full record", 12,
    "100 kyr", "Zumaia", "full record", 4,
    "100 kyr", "Zumaia", "depth < 109.26", 4,
    "100 kyr", "Zumaia", "depth >= 109.26", 4,
    "405 kyr", "Sopelana", "full record", 4,
    "prec", "Zumaia", "depth < 109.26", 0.72,
    "prec", "Zumaia", "depth >= 109.26", 0.49,
    "prec", "Sopelana", "full record", 0.49,
  ) |>
    mutate(f = 1 / p,
           range = freq_perc * f,
...
:END:
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_highpass.png :width 3400 :height 2100 :res 300
  smooth_hp |>
...
#+begin_src R :results none
  smooth_all <- smooth_basic |>
    ...
    ## bind_rows(smooth_hp) |>
    pivot_longer(c(value, scl, lin_rw, lin_gam, lin_fine, lin_coarse, lin_scl_rw, lin_scl_fine, lin_scl_coarse, gam_scl, gam_pred, lin_pred_rw, lin_pred_fine, lin_pred_coarse, lin_gam_pred, lin_gam_scl),
...
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_detrend_types_all.png :width 2400 :height 2200 :res 300 :eval never
  smooth_all |>
    arrange(detrend_type) |>
    filter(!detrend_type %in%
           c("lin_rw", "lin_coarse", "lin_fine", "lin_gam",
             "lin_gam_scl", "lin_gam_pred", "lin_gam_scl")) |>
...
#+RESULTS:
[[file:imgs/Zumaia-Sopelana_detrend_types_all.png]]
...
#+begin_src R
  smooth_pred <- smooth_all |>
    tidylog::filter(detrend_type %in%
                    c("value", "scl",
                      "lin_scl_rw", "lin_scl_fine", "lin_scl_coarse",
                      "gam_scl", ## "lin_gam_scl"
                      "hp_scl"
                      )) |>
...
#+RESULTS:
#+begin_example
filter: removed 48,049 rows (61%), 30,561 rows remaining
...
**** NEXT show how the linear/gam detrending works
:PROPERTIES:
  ...
  specs_lowspec <- smooth_pred |>
    ...
    mutate(lowspec = map(data,
                      ~ . |>
                        select(depth, value) |>
                        astrochron::linterp(genplot = FALSE, verbose = FALSE) |>
                        astrochron::lowspec(genplot = F, verbose = F, output = 1, lowspan = 3) |>
                        as_tibble()
                        )) |>
    select(-data) |>
...
#+RESULTS:
: Error in `mutate()`:
: ℹ In argument: `lowspec = map(...)`.
: Caused by error in `map()`:
: ℹ In index: 13.
: Caused by error in `rfbaseline()`:
: ! could not compute scale of residuals for iter = 3
: Run `rlang::last_trace()` to see where the error occurred.
  ...
  specs_bt <- smooth_pred |>
    ...
    mutate(bt = map(data,
                      ~ . |>
                        ...
                        pull(value) |>
                        TSAUMN::spec.bt(deltat = .5, plot = FALSE))) |>
    select(-data) |>
...
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_mtm.png :width 2600 :height 3200 :res 300 :eval never
  my_filt <- \(d) d |>
  ...
  specs_fancy |>
    ...
    facet_grid(cols = vars(proxy), rows = vars(site, depth_chunk)) +
    scale_y_log10() +
    ...
    scale_colour_manual(values = c("MTM" = "indianred", "LOWSPEC" = "black",
...
#+begin_src R
  filts <- smooth_pred |>
    ...
    # todo: separate target frequencies for different chunks
...
#+RESULTS:
: filter: removed 52,324 rows (50%), 52,324 rows remaining
...
#+RESULTS:
: filter: removed 60,606 rows (67%), 30,303 rows remaining
...
#+begin_src R :results output graphics file :file imgs/Zumaia_MSD-overview.png :width 800 :height 900
  msd |>
    ...
    facet_grid(cols = vars(site, comb), rows = vars(proxy, sol)) +
    ## geom_point() +
...
#+begin_src R :results output graphics file :file imgs/Zumaia_ecc_RMSD.png :width 400 :height 1200
  ...
  msd |>
    ...
    facet_grid(rows = vars(proxy, sol  # , proxy_lag
               ...
               cols = vars(site, detrend_type, comb)) +
    ## geom_line() +
    geom_raster() +
...
|-------+----------+----------------+-------+---------+-------------+-------+-------|
| Lstar | Sopelana | lin_scl_fine   | 1:0.5 |   66000 |          30 | La10b | 0.935 |
| Lstar | Zumaia   | lin_scl_fine   | 1:0.5 |   65980 |        25.5 | La10b | 0.996 |
| Lstar | Sopelana | gam_scl        | 1:0.5 |   65920 |          28 | La10c | 0.876 |
| Lstar | Zumaia   | lin_scl_fine   | 1:0.5 |   65960 |        25.5 | La10c | 1.014 |
| Lstar | Sopelana | gam_scl        | 1:0.5 |   66020 |          27 | ZB18a | 0.884 |
| Lstar | Zumaia   | lin_scl_rw     | 1:0.5 |   65960 |        25.5 | ZB18a | 1.083 |
| Lstar | Sopelana | lin_scl_fine   | 1:0.5 |   66000 |          27 | ZB20a | 0.822 |
| Lstar | Zumaia   | lin_scl_fine   | 1:0.5 |   65920 |        25.5 | ZB20a | 1.055 |
| Lstar | Zumaia   | lin_scl_fine   | 1:0.5 |   65940 |        25.5 | ZB20b | 1.017 |
| Lstar | Sopelana | gam_scl        | 1:0.5 |   66000 |        29.5 | ZB20b | 1.055 |
| Lstar | Sopelana | lin_scl_fine   | 1:0.5 |   65980 |          30 | ZB20c | 1.012 |
| Lstar | Zumaia   | lin_scl_fine   | 1:0.5 |   65940 |        25.5 | ZB20c | 1.111 |
| Lstar | Sopelana | lin_scl_fine   | 1:0.5 |   65980 |          30 | ZB20d | 1.066 |
| Lstar | Zumaia   | lin_scl_fine   | 1:0.5 |   65940 |        25.5 | ZB20d | 1.156 |
| MS    | Zumaia   | lin_scl_fine   | 1:0.5 |   65960 |          26 | La10b | 1.024 |
| MS    | Sopelana | lin_scl_fine   | 1:0.5 |   65920 |        30.5 | La10b | 1.112 |
| MS    | Zumaia   | lin_scl_rw     | 1:0.5 |   65960 |        26.5 | La10c | 1.084 |
| MS    | Sopelana | gam_scl        | 1:0.5 |   65940 |        30.5 | La10c | 1.094 |
| MS    | Zumaia   | lin_scl_rw     | 1:0.5 |   65940 |          26 | ZB18a | 1.103 |
| MS    | Sopelana | lin_scl_fine   | 1:0.5 |   66020 |          29 | ZB18a | 1.111 |
| MS    | Sopelana | gam_scl        | 1:0.5 |   65980 |        29.5 | ZB20a | 1.066 |
| MS    | Zumaia   | lin_scl_fine   | 1:0.5 |   65920 |          26 | ZB20a | 1.143 |
| MS    | Zumaia   | lin_scl_rw     | 1:0.5 |   65920 |          26 | ZB20b | 1.056 |
| MS    | Sopelana | value          | 1:0.5 |   65920 |          24 | ZB20b | 1.106 |
| MS    | Sopelana | lin_scl_fine   | 1:0.5 |   65940 |        29.5 | ZB20c | 1.137 |
| MS    | Zumaia   | lin_scl_rw     | 1:0.5 |   65940 |          26 | ZB20c |  1.14 |
| MS    | Zumaia   | lin_scl_rw     | 1:0.5 |   65940 |          26 | ZB20d | 1.187 |
| MS    | Sopelana | lin_scl_fine   | 1:0.5 |   66020 |          29 | ZB20d | 1.221 |
| d13C  | Zumaia   | lin_scl_fine   | 1:0.5 |   65920 |        26.5 | La10b | 1.052 |
| d13C  | Zumaia   | lin_scl_coarse | 1:0.5 |   66000 |          25 | La10c | 1.068 |
| d13C  | Zumaia   | lin_scl_fine   | 1:0.5 |   65920 |        26.5 | ZB18a | 1.112 |
| d13C  | Zumaia   | lin_scl_coarse | 1:0.5 |   65980 |          25 | ZB20a | 1.067 |
| d13C  | Zumaia   | lin_scl_fine   | 1:0.5 |   65920 |          26 | ZB20b | 1.061 |
| d13C  | Zumaia   | lin_scl_fine   | 1:0.5 |   65920 |        26.5 | ZB20c | 1.113 |
| d13C  | Zumaia   | lin_scl_fine   | 1:0.5 |   65920 |        26.5 | ZB20d | 1.136 |
...
: ℹ In argument: `mtm = purrr::map(.data$data, spectral_analysis, x = age, y = value)`.
...
#+begin_src R :results output graphics file :file imgs/Zumaia_Sopelana_spectra_filters.png :width 2400 :height 2200 :res 300
  ...
  specs_age |>
    ...
    scale_colour_manual(values = c("MTM" = "indianred","FFT" = "darkgray", "MTLS" = "purple")) +
    coord_cartesian(
    ...
    scale_x_continuous(
    ## scale_x_log10(
      breaks = seq(0, 0.070, 0.010),
      minor_breaks = seq(0, 0.070, 0.002),
...
#+begin_src R :results output graphics file :file imgs/Zumaia_Sopelana_records_and_filters.png :width 2400 :height 1800 :res 300
  ...
  filts_age |>
    ...
    # agemodel tie-points
    geom_vline(aes(xintercept = age / 1e3), alpha = .3,
               data = agem_sol |> filter(sol == "ZB18a")) +
...
#+begin_src R
  ...
  tst1 <- data_ecc_age |>
    filter(
      ...
      comb == "1:0", # this doesn't deterimine it, I construct ecc below anyway.
      sol == "ZB18a") |>
    wrap_age_model(
      agemodel = agem_sol |> filter(sol == "ZB18a"),
      astronomical_solution = slns |> filter(sln == "ZB18a"),
      proxy_phase = -1, # -1 for Lstar and d13C, +1 for MS
      eccentricity_weights = c(1, 0.5),
      ...
      max_error_range = 4, #1.6, # 10% of 16 m 405 kyr cycle
...
#+begin_src R :eval never
  ...
  mw <- function(d, s, s405, s100, sign) {
    d |>
      wrap_age_model(
        agemodel = agem_sol |> filter(sol == s),
  ...
  rolling_age_optim <-
    ...
    ## filter(detrend_type == "lin_scl_fine") |> # 315 sims
...
#+begin_src R :results output graphics file :file imgs/new_rolling_improvement.png :width 1700 :height 2600 :res 300

  best_age_optim |>
    filter(site == "Zumaia") |>
    ## filter(site == "Zumaia", comb == "1:1", proxy == "Lstar") |>
    mutate(comb = factor(comb,
                         levels = c("1:0", "1:0.25", "1:0.5", "1:0.75",
                                    "1:1", "0.75:1", "0.5:1", "0.25:1", "0:1"))) |>
      mutate(grp = case_when(
             exp == "Zumaia lin_scl_fine lower-portion 1:1" ~ "depth < 109.26",
             exp == "Zumaia lin_scl_fine upper-portion 1:1" ~ "depth >= 109.26",
             TRUE ~ "full record",
           )) |>
    mutate(grp = factor(grp, levels = c("full record", "depth >= 109.26", "depth < 109.26"))) |>
    filter(grp == "full record") |>
    ggplot(aes(y = sol, x = RMSD, colour = detrend_type,
               group = paste(site, proxy, comb,
                             detrend_type, date, exp))) +
    labs(x = "Root Mean Square Deviation (RMSD)",
         y = "Astronomical solution",
         shape = "Age model",
         linetype = "Age model") +
    facet_grid(cols = vars(proxy), rows = vars(comb),
               scales = "free_x", space = "free_x",
               ) +
    ...
    geom_point(aes(x = RMSD_start, shape = "Field"), size = 3) +
    geom_point(aes(fill = detrend_type, shape = "Optimized"), size = 3) +
    geom_path(aes(x = RMSD_start, linetype = "Field")) +
    geom_path(aes(y = sol, x = RMSD, linetype = "Optimized")) +
    ## geom_segment(arrow=arrow(type = "closed", length = unit(0.1, "inches")))
    geom_linerange(aes(xmin = RMSD_start, xmax = RMSD),
                   alpha = .2)
#+end_src

#+RESULTS:
[[file:imgs/new_rolling_improvement.png]]
...
#+begin_src R :results output graphics file :file imgs/rolling_age_overview_new.png :width 1700 :height 1700 :res 300
  best_age_optim |>
    ...
    mutate(grp = factor(grp, levels = rev(c("depth >= 109.26", "depth < 109.26", "full record")))) |>
    ggplot(aes(x = RMSD, y = sol, colour = detrend_type,
...
#+begin_src R :results output graphics file :file imgs/Zumaia_Lstar_1-1_solutions_simple.pdf :width 9.6 :height 8
  best_age_optim |>
    ...
    filter(proxy == "Lstar") |>
    filter(detrend_type == "lin_scl_fine") |>
    filter(str_detect(exp, "-portion") | site == "Sopelana" | exp == "all after package") |>
    mutate(m = map(the_best, "matched")) |>
    unnest(m) |>
    ...
    geom_text(aes(x = age * 1e-3, y = 4,
              ...
              data = \(x) x |>
                          filter(age == min(age),
                                 .by = c(RMSD, site, sol, proxy, detrend_type))) +
    # annotate depths of 405 kyr minima
    geom_point(aes(x = age * 1e-3, y = -4),
               shape = ".",
              data = best_age_optim |>
                filter(comb == "1:1") |>
                filter(proxy == "Lstar") |>
                filter(detrend_type == "lin_scl_fine") |>
                filter(str_detect(exp, "-portion") | site == "Sopelana") |>
                select(-sol) |>
                mutate(m = map(the_best, "summary")) |>
                unnest(m)) +
    geom_text(aes(x = age * 1e-3, y = -4.5,
...
#+begin_src R :results output graphics file :file imgs/Zumaia_Lstar_1-1_solutions_depth.pdf :width 9.6 :height 2
    best_age_optim |>
      ...
      ## theme(legend.pos = c(.64, .864),
      ##       legend.direction = "horizontal") +
      scale_x_reverse(breaks = seq(200, -50, -10),
