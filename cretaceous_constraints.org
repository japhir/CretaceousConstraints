...
#+begin_src R
  ...
  my_filters <- tribble(
    ...
    mutate(f = 1 / center, range = 0.4 * f, flow = f - range, fhigh = f + range,
           ref = "This study")
#+end_src
...
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_mtm.png :width 900 :height 800
  specs |>
    ...
    ggplot(aes(x = Frequency, y = Power, colour = site, linetype = type, group = type)) +
...
----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ...
 ,* 58 pos/neg frequency pairs will be bandpassed

----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ,* Number of data points= 386
 ,* Sample interval= 0.375
 ,* Mean value removed= 1.563119
 ,* ERROR: maximum frequency too high, reset to Nyquist-df
 ,* Center of bandpass filter = 1.081606
 ,* 144 pos/neg frequency pairs will be bandpassed

----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ,* Number of data points= 1349
 ,* Sample interval= 0.10458
 ,* Mean value removed= 11.55601
 ,* Center of bandpass filter = 0.0625
...
----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ...
 ,* Mean value removed= 11.55601
 ,* Center of bandpass filter = 0.25
 ,* 56 pos/neg frequency pairs will be bandpassed

----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ,* Number of data points= 1349
 ,* Sample interval= 0.10458
 ,* Mean value removed= 11.55601
 ,* Center of bandpass filter = 1.388889
 ,* 313 pos/neg frequency pairs will be bandpassed

----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ,* Number of data points= 1344
 ,* Sample interval= 0.105
 ,* Mean value removed= 49.65039
 ,* Center of bandpass filter = 0.0625
 ,* 14 pos/neg frequency pairs will be bandpassed
...
----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ...
 ,* Mean value removed= 7.718317
 ,* Center of bandpass filter = 0.25
 ,* 20 pos/neg frequency pairs will be bandpassed
...
----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ...
 ,* Mean value removed= 7.718317
 ,* Center of bandpass filter = 1.388889
 ,* 111 pos/neg frequency pairs will be bandpassed

----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ,* Number of data points= 625
 ,* Sample interval= 0.08
 ,* Mean value removed= 55.91772
 ,* Center of bandpass filter = 0.0625
 ,* 5 pos/neg frequency pairs will be bandpassed
...
#+RESULTS:
: indexing 405_minima.csv [==========================================================] 149.95MB/s, eta:  0s                                                                                                                                            Rows: 20 Columns: 5
: ── Column specification ─────────────────────────────────────────────────────────────────────────────────
: Delimiter: ","
...
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_records_simple.png :width 900
  comp |>
    ...
    geom_line(aes(y = filt, colour = "scaled filt"),
              data = filts_scl |>
                filter(target == "405 kyr", site == "Zumaia", name == "MS")) +
...
*** make sure the linear interpolation is consistent between proxies/sites
:PROPERTIES:
...
#+begin_src R
  ...
  smooth |>
    summarize(min(age), .by = c(name, site))

  smooth <- smooth |>
    mutate(dt = case_when(name == "d13C_VPDB" ~ 10,
                          name == "MS" ~ 3,
                          name == "Lstar" ~ 3,
                          TRUE ~ NA_real_),
           strt = case_when(name == "d13C_VPDB" ~ - 91,
                            site == "Zumaia" ~ 2,
                            site == "Sopelana" ~ 3653,
                            TRUE ~ NA_real_))
#+end_src
...
#+begin_src R
  specs_age <- smooth |>
    nest(.by = c(name, site, dt, strt)) |>
    mutate(
      mtm = pmap(list(d = data, dt = dt, strt = strt),
                \(d, dt, strt) d |>
...
#+begin_src R
  filts_age <- smooth |>
    ...
    ## mutate(flow = case_when(name == "MS" ~ 1 / 308,
    ##                         name == "Lstar" ~ 1 / 311,
    ##                         name == "d13C_VPDB" ~ 1 / 326,
    ##                         TRUE ~ NA_real_),
    ##        fhigh = case_when(name == "MS" ~ 1 / 609,
    ##                          name == "Lstar" ~ 1 / 621,
    ##                          name == "d13C_VPDB" ~ 1 / 675,
    ##                          TRUE ~ NA_real_)) |>
    ## filter(!is.na(flow)) |>
    mutate(
  ...
  filts_age_100 <- smooth |>
    ...
    ## mutate(flow = case_when(name == "MS" ~ 1 / 75,
    ##                         name == "Lstar" ~ 1 / 75,
    ##                         ## name == "d13C_VPDB" ~ 1 / 12, # this isn't significant in the spectrum!
    ##                         TRUE ~ NA_real_),
    ##        fhigh = case_when(name == "MS" ~ 1 / 150,
    ##                          name == "Lstar" ~ 1 / 150,
    ##                          ## name == "d13C_VPDB" ~ 1 / 31,
    ##                          TRUE ~ NA_real_)) |>
    ## filter(!is.na(flow)) |>
  ...
  filts_age_prec <- smooth |>
    ...
    ## mutate(flow = case_when(name == "MS" ~ 1 / 17.8,
    ##                         name == "Lstar" ~ 1 / 13.4,
    ##                         name == "d13C_VPDB" ~ 1 / 21.3, # I did these myself
    ##                         TRUE ~ NA_real_),
    ##        fhigh = case_when(name == "MS" ~ 1 / 35.3,
    ##                          name == "Lstar" ~ 1 / 27.2,
    ##                          name == "d13C_VPDB" ~ 1 / 30, # I did these myself
    ##                          TRUE ~ NA_real_)) |>
    filter(!is.na(flow)) |>
    mutate(
      bp = pmap(list(data, flow, fhigh, dt, strt),
...
#+begin_example
----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ,* Number of data points= 391
 ,* Sample interval= 10
 ,* Mean value removed= -0.04740806
 ,* Center of bandpass filter = 0.002469136
 ,* 12 pos/neg frequency pairs will be bandpassed

----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ,* Number of data points= 1273
 ,* Sample interval= 3
 ,* Mean value removed= 0.02838345
 ,* Center of bandpass filter = 0.002469136
 ,* 11 pos/neg frequency pairs will be bandpassed

----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ,* Number of data points= 1273
 ,* Sample interval= 3
 ,* Mean value removed= -0.009897727
 ,* Center of bandpass filter = 0.002469136
 ,* 11 pos/neg frequency pairs will be bandpassed

----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ,* Number of data points= 562
 ,* Sample interval= 3
 ,* Mean value removed= 0.04900637
 ,* Center of bandpass filter = 0.002469136
 ,* 5 pos/neg frequency pairs will be bandpassed
...
----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ...
 ,* Sample interval= 10
 ,* Mean value removed= -0.04740806
 ,* Center of bandpass filter = 0.01
 ,* 47 pos/neg frequency pairs will be bandpassed
...
----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ...
 ,* Mean value removed= 0.02838345
 ,* Center of bandpass filter = 0.01
 ,* 46 pos/neg frequency pairs will be bandpassed
...
----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ...
 ,* Mean value removed= -0.04740806
 ,* ERROR: maximum frequency too high, reset to Nyquist-df
 ,* Center of bandpass filter = 0.04015345
 ,* 153 pos/neg frequency pairs will be bandpassed

----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ,* Number of data points= 1273
 ,* Sample interval= 3
 ,* Mean value removed= 0.02838345
 ,* Center of bandpass filter = 0.04347826
 ,* 199 pos/neg frequency pairs will be bandpassed
...
----- BANDPASS FILTERING STRATIGRAPHIC SERIES-----
 ...
 ,* Mean value removed= 0.04900637
 ,* Center of bandpass filter = 0.04347826
 ,* 88 pos/neg frequency pairs will be bandpassed
...
#+begin_src R
  hb_age_prec <- filts_age_prec |>
    select(-data) |>
    nest(.by = c(site, name, flow, fhigh)) |>
...
----- PERFORMING HILBERT TRANSFORM ON STRATIGRAPHIC SERIES -----
 ,* Number of data points= 391
 ,* Sample interval= 10
 ,* Mean value removed= -0.04701458

----- PERFORMING HILBERT TRANSFORM ON STRATIGRAPHIC SERIES -----
 ,* Number of data points= 1273
 ,* Sample interval= 3
 ,* Mean value removed= 0.02782609

----- PERFORMING HILBERT TRANSFORM ON STRATIGRAPHIC SERIES -----
 ,* Number of data points= 1273
 ,* Sample interval= 3
 ,* Mean value removed= -0.009172407

----- PERFORMING HILBERT TRANSFORM ON STRATIGRAPHIC SERIES -----
 ,* Number of data points= 562
 ,* Sample interval= 3
 ,* Mean value removed= 0.04891497

----- PERFORMING HILBERT TRANSFORM ON STRATIGRAPHIC SERIES -----
 ,* Number of data points= 562
 ,* Sample interval= 3
 ,* Mean value removed= -0.01865478
#+end_example
...
#+begin_src R :results output graphics file :file imgs/Zumaia_Sopelana_spectra_filters.png :width 900
  ...
  specs_age |>
    ...
    coord_cartesian(xlim = c(1 / 5000, 1 / 10)) +
    scale_y_log10() +
    ## scale_x_log10(
    scale_x_continuous(
...
#+begin_src R :eval never
  ...
  specs_age |>
    ...
    geom_rect(aes(xmin = 1 / flow, xmax = 1 / fhigh, ymin = 1e-5, ymax = 1),
              inherit.aes = FALSE,
              alpha = .3, fill = "yellow",
              data = filts_age |> distinct(name, flow, fhigh)) +
...
#+begin_src R :results output graphics file :file imgs/Zumaia_Sopelana_records_and_filters.png :width 900
  smooth |>
    ## mutate(det = scale(value)[, 1], .by = c(site, name)) |>
    ...
    geom_point(aes(y = pk + 1.5),
               colour = "red",
               data = filts_age |>
                            nest(.by = c(name, site, flow, fhigh)) |>
...
#+RESULTS:
#+begin_example
Joining with `by = join_by(name, site, age)`
left_join: added one column (100)
rows only in x       0
rows only in y  (    0)
matched rows     4,061
                =======
rows total       4,061
Joining with `by = join_by(name, site, age)`
left_join: added one column (envelope)
rows only in x       0
rows only in y  (    0)
matched rows     4,061
                =======
...
:END:
#+begin_src R
  # construct a fake eccentricity record
  smooth |>
    ## mutate(det = scale(value)[, 1], .by = c(site, name)) |>
...
#+RESULTS:
| proxy     | site     | name          |            value |
|-----------+----------+---------------+------------------|
| d13C_VPDB | Zumaia   | msd_1_2       | 29.3598821079939 |
| d13C_VPDB | Zumaia   | msd_1_1.5     | 29.6581384794546 |
| d13C_VPDB | Zumaia   | msd_1_1       | 29.9846266540965 |
| d13C_VPDB | Zumaia   | msd_1_0.5     | 30.2994787661913 |
| d13C_VPDB | Zumaia   | msd_1_2_env   | 30.3188077230196 |
| d13C_VPDB | Zumaia   | msd_1_1.5_env | 30.5381119730474 |
| d13C_VPDB | Zumaia   | msd_1_1_env   | 30.7380319186259 |
| d13C_VPDB | Zumaia   | msd_1_0.5_env | 30.7867245290165 |
| MS        | Sopelana | msd_1_2       | 31.1290307431718 |
| MS        | Sopelana | msd_1_1.5     | 31.2806309294031 |
| MS        | Sopelana | msd_1_1       | 31.5979866131033 |
| Lstar     | Sopelana | msd_1_2_env   | 32.0109455469781 |
| MS        | Sopelana | msd_1_0.5     | 32.4996960192738 |
| Lstar     | Sopelana | msd_1_1.5_env | 32.5838862334169 |
| MS        | Sopelana | msd_1_2_env   | 33.1162819006392 |
| MS        | Sopelana | msd_1_1.5_env |  33.220217937339 |
| MS        | Sopelana | msd_1_1_env   | 33.4146645790638 |
| Lstar     | Sopelana | msd_1_1_env   | 33.6447657872902 |
| MS        | Sopelana | msd_1_0.5_env | 33.8834752737462 |
| Lstar     | Sopelana | msd_1_2       | 34.9674358794334 |
| Lstar     | Sopelana | msd_1_1.5     | 35.4537885662001 |
| Lstar     | Sopelana | msd_1_0.5_env | 35.8804613843147 |
| Lstar     | Sopelana | msd_1_1       | 36.2790314022912 |
| Lstar     | Sopelana | msd_1_0.5     | 37.7416472597871 |
| Lstar     | Zumaia   | msd_1_2_env   | 44.8509657385971 |
| Lstar     | Zumaia   | msd_1_1.5_env | 45.0940488838285 |
| Lstar     | Zumaia   | msd_1_1_env   | 45.6709012369143 |
| Lstar     | Zumaia   | msd_1_0.5_env | 47.0660787794822 |
| MS        | Zumaia   | msd_1_2_env   | 47.1751100097964 |
| MS        | Zumaia   | msd_1_1.5_env | 47.4342665569233 |
| MS        | Zumaia   | msd_1_1_env   | 47.9225909931922 |
| MS        | Zumaia   | msd_1_0.5_env | 48.9197030810149 |
| MS        | Zumaia   | msd_1_0.5     | 51.6594934150535 |
| MS        | Zumaia   | msd_1_1       | 52.2665203096134 |
| Lstar     | Zumaia   | msd_1_0.5     | 52.2858202454809 |
| MS        | Zumaia   | msd_1_1.5     | 52.6195669732602 |
| MS        | Zumaia   | msd_1_2       | 52.8215096044653 |
| Lstar     | Zumaia   | msd_1_1       | 53.4534743041263 |
| Lstar     | Zumaia   | msd_1_1.5     | 53.9403036679462 |
| Lstar     | Zumaia   | msd_1_2       | 54.1797169740796 |
...
#+begin_src R :results output graphics file :file imgs/Zumaia_time-slider.png :width 900
  data_ecc_off_summ |>
    ggplot(aes(x = age_kpg, y = value, colour = name, group = paste(site, name))) +
    geom_line() +
