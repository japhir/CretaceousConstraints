...
#+RESULTS:
#+begin_example
-/                                                                                                       /                                                                                                       -                                                                                                       > > > + + + indexing 18696_2.csv [===============================================================] 1.83GB/s, eta:  0s                                                                                                                                            New names:
• `` -> `...3`
Rows: 629 Columns: 3
── Column specification ─────────────────────────────────────────────────────────────────────────────────
Delimiter: ","
...
#+begin_src R :exports none
  ...
  agem_simple <- agem |>
    ...
    unnest(cols = c(sol)) |>
    nest(.by = sol) |>
    # TODO: this is still wrong, strat = 0 = K/Pg boundary
    ...
    arrange(sol, n) |>
    mutate(age_diff = abs(age - lead(age)), .by = sol)
...
#+RESULTS:
: indexing 405_minima.csv [==========================================================] 342.73MB/s, eta:  0s                                                                                                                                            Rows: 20 Columns: 5
: ── Column specification ─────────────────────────────────────────────────────────────────────────────────
: Delimiter: ","
...
****** also do simple FFT periodogram
:PROPERTIES:
...
#+end_src
****** SOME calculate the BT spectrum as well
:PROPERTIES:
...
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_mtm.png :width 900 :height 800
  ...
  specs_fancy |>
    ...
    # plot periodogram
    geom_line(aes(linetype = "FFT"), data = specs_fft) +
    ...
    geom_line(aes(y = spec, linetype = "MTLS"), data = specs_mtls) +
    scale_y_log10() +
    ...
    labs(x = "Frequency (cycles/m)", y = "Spectral power",
         linetype = "") +
    scale_linetype_manual(values = c("MTM" = 1,"FFT" = 2, "MTLS" = 3))
#+end_src
...
#+begin_src R :results value :colnames yes
  best <- msd |>
    ...
    tidylog::filter(RMSD == min(RMSD), .by = c(proxy, site, comb, filt_env, kpg_age, sol))
#+end_src
...
:END:
This becomes [[cref:fig:best-depth]].
...
#+name: fig:best-depth
#+caption: Zumaia best proxy match with astronomical solutions based on single sedimentation rate.
#+attr_latex: :width \linewidth
#+RESULTS:
...
[2023-11-15 Wed 14:31]
While typing this, I realize I could also first apply the age model and then do the filtering in the time domain rather than squishing the filtered record... Hmmmm...
...
#+RESULTS:
: filter: removed 21,645 rows (83%), 4,329 rows remaining
*** use eTimeOpt to estimate sedimentation rates
:PROPERTIES:
...
#+begin_src R :results none
  best_tie <- msd_tie |>
    ...
    tidylog::filter(RMSD == min(RMSD), .by = c(proxy, site, comb, filt_env, sol))
