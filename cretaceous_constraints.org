...
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_mtm.png :width 2900 :height 2200 :res 300
  my_filt <- \(d) d |>
    ## filter(detrend_type != "value") |>
  ...
  specs_fancy |>
    ...
    facet_grid(cols = vars(proxy), rows = vars(site, depth_chunk, detrend_type)) +
    scale_y_log10() +
...
#+RESULTS:
: filter: removed 104,648 rows (67%), 52,324 rows remaining
