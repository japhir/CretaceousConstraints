...
#+begin_src R :results output graphics file :file imgs/detrend_d13C.png :width 800 :height 450
  pl_detrend_d13C <- smooth_all |>
    ...
    ##                                          "Lstar" = "L*")),
    ##            switch = "y") +
    labs(x = "Depth (m)", y = delta^{13}*C~"(\u2030 VPDB)") +
    coord_cartesian(xlim = c(183, 0)) +
    scale_x_reverse(breaks = seq(-20, 200, 20),
                    minor_breaks = seq(-10, 200, 10)) +
    scale_colour_manual(name = "",
...
#+begin_src R
  rolling_age_optim <-
    bind_rows(
      read_rds("out/2024-03-21_rolling_age_optim_all_Lstar_one-type_taner.rds"),
      read_rds("out/2024-03-21_rolling_age_optim_all_other-proxies_one-type_taner.rds")
    ) |>
    mutate(detrend_type = as_factor(detrend_type) |>
             fct_relevel(names(detrend_colours))) |>
    mutate(comb = as_factor(comb) |>
...
: Caused by warning:
: ! 7 unknown levels in `f`: scl, gam_scl, hp_scl, lin_scl_rw, lin_scl_fine, lin_scl_coarse, and value
...
: filter: removed 368 rows (1%), 26,188 rows remaining
...
#+begin_src R :results output graphics file :file imgs/full_RMSD_scores.pdf :width 4 :height 2
  pl_rmsd_full <- overall_best |>
    ...
    ggplot(aes(x = sol, y = RMSD,
               ## colour = proxy#col = detrend_type## , shape = window
               )) +
    labs(x = "Astronomical Solution",## , shape = "Bandpass Filter type"
         colour = ""
         ) +
    ...
    ##           alpha = .1) +
    ## geom_point(alpha = .05) +
    geom_line(aes(group = proxy, ## colour = proxy
                  ), alpha = .3,
              data = \(x) x |>
                          filter(RMSD == min(RMSD), .by = c(sol, proxy))) +
    geom_point(data = \(x) x |>
...
:END:
#+begin_src R :results output graphics file :file imgs/cumulative_rRMSD_all.pdf :width 5 :height 5
  overall_sd |>
    tidylog::inner_join(best_age_optim_one |> select(-the_best)) |>
    ## filter(proxy == "Lstar") |>
    filter(depth_chunk == "full record") |>
...
**** [#B] simplified plot of raw data vs depth
:PROPERTIES:
:CREATED:  [2024-01-15 Mon 15:22]
:END:
I should:
...
#+begin_src R
  ...
  simp <- simp |>
    ...
    tidylog::filter(!(site == "Zumaia" & depth > min_depth))
#+end_src
...
[[file:~/SurfDrive/Postdoc1/prj/2023-05-19_cretaceous_constraints/cretaceous_constraints.org::*simplified plot of raw data vs depth][simplified plot of raw data vs depth]]
