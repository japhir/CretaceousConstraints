...
#+begin_src R
  wrap_age_model <- function(data,
    ...
    the_best <- agemodel |>
      assertr::verify(is_same(site),
                      error_fun = stop("`site` must contain only one site")) |>
      assertr::verify(is_same(sol),
                      error_fun = stop("`sol` must contain only one Astronomical Solution")) |>
      rename(Ma405 = n) |>
      select(all_of(c("sol", "site", "Ma405", "strat_bot", "age"))) |>
...
#+begin_src R :results output graphics file :file imgs/Zumaia_fixed_d13C.png :width 900 :height 700
  ...
  wrap_age_model(subs, agem_sol |> filter(site == "Zumaia", sol == "ZB18a"),
...
**** try to call this function on the full dataframe
:PROPERTIES:
...
#+begin_src R
  ...
  the_superduper_best <- diffs_tie |>
    ...
    ## filter(proxy %in% c("Lstar", "MS"),
    ##        detrend_type %in% c("scl", "lin_gam_det"),
    ##        comb %in% c("1:1", "0.5:1"),
    ##        sol %in% c("ZB18a", "ZB20a")) |>
    nest(.by = c(proxy, site, detrend_type, comb, filt_env, sol)) |>
    slice(341) |> # sopelana wasn't working
    mutate(the_best = furrr::future_pmap( # use cores
                               .l = list(data, sol, site),
                               .f = ~ ..1 |>
    ...
    ## write_rds("out/the_superduper_best.rds")
#+end_src
