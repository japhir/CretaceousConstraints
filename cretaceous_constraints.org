...
#+begin_src R :eval never
  ...
  rolling_age_optim <-
    ...
    write_rds("out/2024-02-21_rolling_age_optim_all_remainder_gaussian.rds")
  message("current time: ", lubridate::now())
...
#+begin_src R
  rolling_age_optim <-
    bind_rows(
      ...
      read_rds("out/2024-02-21_rolling_age_optim_all_remainder_rectangular.rds"),
      ) |>
    mutate(window = "rectangular") |>
    bind_rows(
      bind_rows(
        ...
        read_rds("out/2024-02-21_rolling_age_optim_all_remainder_gaussian.rds"),
        ) |>
...
#+begin_src R
  best_age_optim <- rolling_age_optim |>
    ...
    tidylog::filter(!(depth_chunk == "depth >= 109.26" & detrend_type == "lin_scl_fine" & sol == "ZB20a")) |>
...
: filter: removed 5 rows (1%), 576 rows remaining
: filter: removed 7 rows (1%), 569 rows remaining
...
#+begin_src R
  best_age_optim_one <- best_age_optim |>
    filter(RMSD == min(RMSD), .by = c(sol, proxy, site, comb, depth_chunk))
#+end_src
...
#+begin_src R :results output graphics file :file imgs/new_rolling_improvement.png :width 2400 :height 2000 :res 300
  best_age_optim |>
    ...
    geom_point(aes(alpha = window), size = 3) +
    geom_path(aes(y = RMSD_start, linetype = "Field")) +
    geom_path(aes(x = sol, y = RMSD, alpha = window, linetype = "Optimized")) +
...
#+begin_src R
  my_filt <- \(x) x |>
                    ...
                    tidylog::distinct(sol, site, proxy, depth_chunk, comb, window,
                             .keep_all = TRUE)
...
#+begin_src R :results output graphics file :file imgs/Zumaia_Lstar_1-1_solutions_simple.pdf :width 9.6 :height 8
  best_age_optim |>
    ...
    scale_alpha_continuous(range = c(.6, 0.2)) +
    scale_colour_manual(values = c("Astronomical Solution" = "black",
                                   "Zumaia" = "purple",
                                   "Sopelana" = "skyblue",
                                   ## "Lstar" = "purple",
    ...
    geom_errorbarh(aes(xmin = lwr * 1e-3,
                       xmax = upr * 1e-3,
                       y = -7 + 14 * as.integer(depth_chunk == "full record")),
                   height = 1, alpha = .2,
    ...
    geom_text(aes(x = age * 1e-3, y = -5 + 10 * as.integer(depth_chunk == "full record"), #y = 4 + as.integer(depth_chunk),
                  label = round(RMSD, 3)),
              ...
              data = best_age_optim_one |>
                ...
                mutate(age = map_dbl(the_best,
                                     \(x) mean(x$matched$age,
                                                 na.rm = TRUE)))) +
    geom_text(aes(x = age * 1e-3, y = -6 + 12 * as.integer(depth_chunk == "full record"),# + as.integer(depth_chunk),
                  label = paste(detrend_type, window)),
              size = 2, show.legend = FALSE,
              data = best_age_optim_one |>
                my_filt() |>
                mutate(age = map_dbl(the_best,
