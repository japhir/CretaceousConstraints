...
| ~lowpass_det~  | Subtract a lowpass filter from the value, then scale the result.               |
...
#+begin_src R :results none
  smooth_all <- smooth_basic |>
    ...
    ## bind_rows(smooth_hp) |>
    pivot_longer(c(value, scl, lin, lin_gam, lin_fine, lin_coarse, lin_scl_old, lin_scl_fine, lin_scl_coarse, gam_scl, gam_pred, lin_pred, lin_pred_fine, lin_pred_coarse, lin_gam_pred, lin_gam_scl),
...
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_detrend_types_all.png :width 2400 :height 2200 :res 300
  smooth_all |>
    ...
    filter(!detrend_type %in%
           c("lin", "lin_coarse", "lin_fine", "lin_gam",
...
#+begin_src R
  smooth_pred <- smooth_all |>
    ...
    tidylog::filter(detrend_type %in%
                    c("value", "scl",
                      "lin_scl_old", "lin_scl_fine", "lin_scl_coarse",
                      "gam_scl"#, ## "lin_gam_det"
                      ))
...
#+RESULTS:
#+begin_example
filter: removed 52,378 rows (67%), 26,232 rows remaining
# A tibble: 6 × 1
  detrend_type
  <chr>
1 value
...
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_records_complicated.png :height 1200 :width 2600 :res 300
  ...
  smooth |>
    ...
    annotate("text", x = xnot, y = -30, label = "lin_scl",
             size = 2, hjust = 1) +
    ...
    annotate("text", x = xnot, y = -20, label = "highpass = scale(value-lowpass)",
             size = 2, hjust = 1) +
    ...
    geom_line(aes(y = lin_pred, colour = "lin", group = colour),
              linewidth = 1.2, alpha = .6,
              data = smooth |>
                filter(site == "Zumaia", proxy == prx)) +
...
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_MS_wavelet.png :width 2400 :height 1200 :res 300
  ## smooth_wavelet <-
    ...
    ## filter(detrend_type == "value") |>
    ## filter(detrend_type == "scl") |>
    ## filter(detrend_type == "lin_scl_old") |>
...
#+RESULTS:
: filter: removed 51,948 rows (67%), 25,974 rows remaining
...
# A tibble: 35 × 4
   proxy site     sol    RMSD
   <chr> <chr>    <chr> <dbl>
 1 Lstar Sopelana La10b  1.58
 2 Lstar Sopelana La10c  1.59
 3 Lstar Sopelana ZB18a  1.70
 4 Lstar Sopelana ZB20a  1.33
 5 Lstar Sopelana ZB20b  1.66
 6 Lstar Sopelana ZB20c  1.72
 7 Lstar Sopelana ZB20d  1.72
 8 MS    Sopelana La10b  1.63
 9 MS    Sopelana La10c  1.45
...
#+RESULTS:
***** figure of the heart of the matching algorithm
:PROPERTIES:
:CREATED:  [2024-01-10 Wed 16:01]
:END:
[[file:imgs/Zumaia_Lstar_filter_matching.png]]

[[file:imgs/Zumaia_MS_filter_matching.png]]
***** NEXT Zumaia tests
:PROPERTIES:
...
#+begin_src R :eval never
  ...
  rolling_age_optim <-
    ...
    ## filter(detrend_type == "lin_scl_fine") |> # the first full run!
    filter(!detrend_type %in% c("lin_scl_fine", "gam_scl", "value"),
           proxy == "Lstar", site == "Zumaia", comb == "1:1") |>
    # this gives me 300 sims!
    nest(.by = c(proxy, site,
    ...
    write_rds("out/2024-01-12_rolling_age_optim_Zumaia_Lstar_detrend_types.rds") # after making it a package
  message("current time: ", lubridate::now())
#+end_src
...
this is 300 rows rather than 100, and saves way more results. so might be quite a bit slower/bigger.
...
#+begin_src R
  rolling_age_optim <-
    ...
    bind_rows(
      # new algorithm, all of lin_scl_fine
      read_rds("out/2024-01-12_rolling_age_optim_full.rds"),
      # some alternative detrend_types for a subset of runs
...
improvement after optimization after algorithm revision
