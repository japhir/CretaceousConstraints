...
: filter: removed 16,988 rows (37%), 28,659 rows remaining
...
:END:
#+begin_src R :results output graphics file :file imgs/cumulative_rRMSD.pdf :width 5 :height 3
  overall_sd |>
    tidylog::inner_join(best_age_optim_one |> select(-the_best)) |>
    filter(proxy == "Lstar") |>
    mutate(rRMSD = sqrt(cumsum(SD)),
...
#+RESULTS:
[[file:imgs/cumulative_rRMSD.pdf]]
