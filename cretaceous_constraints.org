...
#+RESULTS:
: indexing ODP208_1267_MS.csv [============-------------------------------------------] 37.98GB/s, eta:  0sindexing ODP208_1267_MS.csv [======================================================] 237.15MB/s, eta:  0s                                                                                                                                            Rows: 23239 Columns: 28
: ── Column specification ─────────────────────────────────────────────────────────────────────────────────
: Delimiter: ","
...
#+begin_src R :results graphics file :file imgs/ODP208_1267_MS_mtm.png :width 800
  ...
  MS_ |>
    ...
    mutate(spec = map(data, \(d) d |>
                                 select(my_rmcd, value) |>
                                 astrochron::linterp(genplot = FALSE, verbose = FALSE) |>
                                 astrochron::mtm(output = 1, detrend = TRUE, demean = TRUE,
                                                 xmax = 5, genplot = FALSE, verbose = FALSE)
                      )) |>
    unnest(spec) |>
    ggplot(aes(x = Frequency, y = Power, colour = smooth_type)) +
    geom_line() +
    geom_line(aes(y = AR1_fit)) +
    geom_line(aes(y = AR1_95_power), alpha = .3) +
    scale_y_log10() +
    annotation_logticks() +
    coord_cartesian(xlim = c(1e-4, 0.0625) * 100, ylim = c(1e-5, 1)) +
    ## scale_x_continuous(
    scale_x_log10(
      sec.axis = sec_axis(trans = ~ 1 / .x,
                          name = "Period (m)",
    ...
    geom_rect(aes(xmin = flow, xmax = fhigh, ymin = 1e-5, ymax = 1, fill = ref),
              inherit.aes = FALSE,
              alpha = .3, data = hus_filters) +
    geom_rect(aes(xmin = flow, xmax = fhigh, ymin = 1e-5, ymax = 1, fill = ref),
              inherit.aes = FALSE,
              alpha = .3, data = my_1267_filters) +
    labs(x = "Frequency (cycle/m)", y = "Spectral power")
#+end_src
...
I've various ways of detrending the record first but it's not coming out too clearly.
...
#+begin_src R :exports none
  ...
  comp <- Zumaia |>
    pivot_longer(MS:d13C_VPDB, names_to = "proxy") |>
    mutate(comment = if_else(proxy == "d13C_VPDB", comment, NA_character_)) |>
...
#+RESULTS:
#+begin_example
-/                                                                                                       /                                                                                                       -                                                                                                       > > > + + + indexing 18696_2.csv [===============================================================] 1.26GB/s, eta:  0s                                                                                                                                            New names:
• `` -> `...3`
Rows: 629 Columns: 3
── Column specification ─────────────────────────────────────────────────────────────────────────────────
Delimiter: ","
...
:END:
**** lines
#+begin_src R :results output graphics file :file imgs/Zumaia_linear-trend-removal.png :width 900
  smooth |>
...
**** gam
#+begin_src R :results output graphics file :file imgs/Zumaia_gam-trend-removal.png :width 900
  smooth |>
...
**** detrended result
#+begin_src R :results output graphics file :file imgs/Zumaia_linear-gam-trend-removal.png :width 900
  smooth |>
...
#+begin_src R :results output graphics file :file imgs/Zumaia_ecc_ZB18a.png :width 900
  ...
  diffs |>
    ...
    ## summarize(MSD = sqrt(sum(diff)), .by = c(proxy, site, comb, filt_env, tau, kpg_age, sol)) |>
    ## arrange(MSD)
...
#+begin_src R :results value :colnames yes
  smooth <- smooth |>
    mutate(dt = case_when(proxy == "d13C_VPDB" ~ 10,
                          proxy == "MS" ~ 3,
                          proxy == "Lstar" ~ 3,
                          TRUE ~ NA_real_),
           strt = case_when(proxy == "d13C_VPDB" ~ - 91,
                            site == "Zumaia" ~ 2,
                            site == "Sopelana" ~ 3653,
                            TRUE ~ NA_real_))
#+end_src
...
#+begin_src R :results none
  filts_age <- smooth |>
    ...
    ##                         proxy == "Lstar" ~ 1 / 311,
    ##                         proxy == "d13C_VPDB" ~ 1 / 326,
    ##                         TRUE ~ NA_real_),
    ##        fhigh = case_when(proxy == "MS" ~ 1 / 609,
    ##                          proxy == "Lstar" ~ 1 / 621,
    ##                          proxy == "d13C_VPDB" ~ 1 / 675,
    ##                          TRUE ~ NA_real_)) |>
    ## filter(!is.na(flow)) |>
    mutate(
  ...
  filts_age_100 <- smooth |>
    ...
    ##                         proxy == "Lstar" ~ 1 / 75,
    ##                         ## proxy == "d13C_VPDB" ~ 1 / 12, # this isn't significant in the spectrum!
    ##                         TRUE ~ NA_real_),
    ##        fhigh = case_when(proxy == "MS" ~ 1 / 150,
    ##                          proxy == "Lstar" ~ 1 / 150,
    ##                          ## proxy == "d13C_VPDB" ~ 1 / 31,
    ##                          TRUE ~ NA_real_)) |>
    ## filter(!is.na(flow)) |>
    mutate(
      bp = pmap(list(data, flow, fhigh, dt, strt),
  ...
  filts_age_prec <- smooth |>
    ...
    ##                         proxy == "Lstar" ~ 1 / 13.4,
    ##                         proxy == "d13C_VPDB" ~ 1 / 21.3, # I did these myself
    ##                         TRUE ~ NA_real_),
    ##        fhigh = case_when(proxy == "MS" ~ 1 / 35.3,
    ##                          proxy == "Lstar" ~ 1 / 27.2,
    ##                          proxy == "d13C_VPDB" ~ 1 / 30, # I did these myself
    ##                          TRUE ~ NA_real_)) |>
    filter(!is.na(flow)) |>
    mutate(
      bp = pmap(list(data, flow, fhigh, dt, strt),
