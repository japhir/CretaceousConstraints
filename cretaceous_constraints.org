...
#+begin_src R :results output graphics file :file imgs/new_rolling_improvement.png :width 2400 :height 2000 :res 300
  ...
  best_age_optim |>
    ...
    labs(
      ...
      shape = "Age model",
      linetype = "Age model") +
    facet_grid(cols = vars(proxy), rows = vars(site, depth_chunk,comb),
    ...
    geom_point(aes(colour = detrend_type, shape = "Optimized"), size = 4) +
    geom_path(aes(y = RMSD_start, linetype = "Field")) +
    geom_path(aes(x = sol, y = RMSD, linetype = "Optimized")) +
