...
#+RESULTS:
: indexing 405_minima.csv [==========================================================] 436.21MB/s, eta:  0s                                                                                                                                            Rows: 20 Columns: 5
: ── Column specification ─────────────────────────────────────────────────────────────────────────────────
: Delimiter: ","
...
#+begin_src R
  ...
  diffs <- data_ecc |>
    ...
    unnest(cols = c(tau)) |>
    mutate(age = kpg_age + depth * tau + proxy_lag) |>
...
#+begin_src R :results output graphics file :file imgs/Zumaia_MSD-overview.png :width 800
  msd |>
    ggplot(aes(x = kpg_age * 0.001,
               y = RMSD,
               colour = paste(tau, " = ", round(1 / tau * 100, 2))## ,
               ## linetype = comb
               )) +
    ...
    geom_line(aes(group = paste(tau, comb, filt_env, proxy_lag)), linewidth = .7
              ) +
...
#+RESULTS:
| proxy | site   |  comb | filt_env |  tau | kpg_age | proxy_lag | sol   |             RMSD |
|-------+--------+-------+----------+------+---------+-----------+-------+------------------|
...
#+begin_src R :results output graphics file :file imgs/Zumaia_ecc_ZB18a.png :width 900
  ...
  diffs |>
    ...
    ggplot(aes(x = age * 0.001,
               group = paste(comb, filt_env, site, tau, kpg_age, proxy_lag),
               alpha = RMSD,
...
#+begin_src R
  ...
  diffs_tie <- data_ecc |>
    ...
    mutate(proxy_lag = case_when(
             ...
             TRUE ~ list(0.))) |>
    unnest(cols = c(proxy_lag)) |>
    # I don't think I'm using tau here, I'm linearly interpolating between subsequent 405 kyr minima
    ## mutate(tau = list(seq(
    ...
    mutate(tie_age = map2_dbl(
             depth, tie_err,
             ~ Hmisc::approxExtrap(agem_tie$strat_adj[agem_tie$tie_err == .y],
                                   agem_tie$age[agem_tie$tie_err == .y],
                                   xout = .x)$y)) |>
    # add the number of the current 405 kyr cycle to group by (rounded down)
    mutate(age = kpg_age + tie_age + proxy_lag) |>
    # add target orbital solutions
    mutate(sol = list(c("ZB18a", "ZB20a", "La11"))) |>
    unnest(cols = c(sol)) |>
    mutate(ecc_sln = case_when(
...
#+begin_src R :results none
  best_tie <- msd_tie |>
    filter(site == "Zumaia", proxy == "MS") |>
    filter(sol == "ZB20a", kpg_age == 65920, proxy_lag == 5) |>
