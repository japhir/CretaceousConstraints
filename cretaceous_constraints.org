...
**** NEXT CaCO3: ten Kate and Sprenger 1993
:PROPERTIES:
...
2: Removed 49 rows containing missing values (`geom_line()`).
3: Removed 49 rows containing missing values (`geom_line()`).
Scale for x is already present.
Adding another scale for x, which will replace the existing scale.
Error in `mutate()`:
ℹ In argument: `lt = map(...)`.
