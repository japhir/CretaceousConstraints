...
#+begin_src R :results output graphics file :file imgs/Zumaia_CaCO3_bandpass_filters.png :width 2400 :height 1000 :res 300
  Zumaia_CaCO3 |>
    ...
    labs(x = "Age (Ma) but OLD age model",
...
src_R{1/(agem_sol[agem_sol$sol=="ZB18a","depth_diff"] / 405 * 2000)$depth_diff} {{{results(= [1] 0.01265625 0.01265625 0.01350000 0.01446429 0.01350000 0.01350000 0.01012500 0.01840909 0.01557692 0.01446429 0.01840909 0.01840909 0.01687500         NA=)}}}
...
- Note taken on [2024-01-09 Tue 12:00] \\
  yay finished this?
:END:
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_lowpass_spectra.png :width 2400 :height 2200 :res 300
  spec_hp <- smooth_hp |>
...
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_mtm.png :width 2900 :height 3200 :res 300
  specs_fancy |>
    filter(detrend_type != "value") |>
    ggplot(aes(x = frequency, y = power,
               ## colour = detrend_type,
               group = paste(detrend_type, proxy, site))) +
    facet_grid(cols = vars(proxy), rows = vars(site, detrend_type)) +
    scale_y_log10() +
    ...
    geom_rect(aes(xmin = flow, xmax = fhigh, ymin = 1.5e-4, ymax = 3e-4, fill = ref),
              inherit.aes = FALSE,
              ## colour = "black",
              alpha = .2,
              data = bat_filters |> bind_rows(bat_filters_old)) +
    # show our filters
    geom_rect(aes(xmin = flow, xmax = fhigh, ymin = 1e-5, ymax = 1.5e-4, fill = ref),
              inherit.aes = FALSE,
              alpha = .3,
              data = my_filters) +
    ggnewscale::new_scale_fill() +
    ...
    ##           data = specs_bt|> filter(detrend_type != "value")) +
    # plot MTM spectra
...
| MS    | Zumaia   | lin_scl_rw     | 1:0.5 |   65940 |          26 | ZB18a | 1.104 |
...
| d13C  | Zumaia   | lin_scl_fine   | 1:0.5 |   65920 |        26.5 | ZB18a | 1.113 |
...
|-------+--------------+-------+---------+-------+-------|
| Lstar | lin_scl_fine | 1:0.5 |   65980 | La10b | 0.996 |
| Lstar | lin_scl_fine | 1:0.5 |   65960 | La10c | 1.014 |
| Lstar | lin_scl_fine | 1:0.5 |   65940 | ZB20b | 1.017 |
| Lstar | lin_scl_fine | 1:0.5 |   65920 | ZB20a | 1.055 |
| Lstar | lin_scl_rw   | 1:0.5 |   65960 | ZB18a | 1.083 |
| Lstar | lin_scl_fine | 1:0.5 |   65940 | ZB20c | 1.111 |
| d13C  | lin_scl_fine | 1:0.5 |   65920 | ZB20d | 1.136 |
...
#+begin_src R :results output graphics file :file imgs/new_rolling_improvement.png :width 3400 :height 1700 :res 300
  ...
  best_age_optim |>
    ...
    labs(y = "Astronomical solution", shape = "Age model", linetype = "Age model") +
    facet_grid(rows = vars(proxy), cols = vars(comb),
               ## scales = "free_x"
               ## space = "free_x",
               ) +
    ...
    scale_shape_manual(values = c("Field" = 16, "Optimized" = 25)) +
    scale_linetype_manual(values = c("Field" = "dashed", "Optimized" = "solid")) +
    geom_point(aes(x = RMSD_start, shape = "Field")) +
    geom_point(aes(fill = detrend_type, shape = "Optimized")) +
...
#+begin_src R :results output graphics file :file imgs/rolling_age_overview_new.png :width 2600 :height 1700 :res 300
  best_age_optim |>
    filter(comb == "1:1") |>
    filter(detrend_type != "lin_scl_rw") |>
    ggplot(aes(x = RMSD, y = sol, colour = detrend_type,
               ## alpha = !str_detect(exp, "upper"),
               group = paste(site, proxy, comb,
                             detrend_type, date, exp))) +
    labs(y = "Astronomical solution",
         shape = "Age model", linetype = "Age model"## ,
         ## alpha = "lower / full record"
         ) +
    facet_grid(rows = vars(proxy),
               cols = vars(site, str_detect(exp, "-portion"))) +
    scale_shape_manual(values = c("Field" = 16, "Optimized" = 25)) +
    scale_linetype_manual(values = c("Field" = "dashed", "Optimized" = "solid")) +
    geom_point(aes(x = RMSD_start, shape = "Field")) +
    geom_point(aes(fill = detrend_type, shape = "Optimized")) +
...
#+RESULTS:
: filter: removed 28 rows (12%), 203 rows remaining
  ...
  best_age_optim |>
    ...
    ggplot(aes(x = depth,
               ...
               group = paste(site, sol, proxy,
                             detrend_type, date, exp))) +
    facet_grid(rows = vars(sol)) +
    labs(x = "Depth (m)", y = "Norm. Filter & Solution",
         colour = "") +
    ## theme(legend.pos = c(.64, .864),
    ##       legend.direction = "horizontal") +
