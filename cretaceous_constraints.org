...
#+begin_src R
  ...
  wrap_age_model <- function(data,
    ...
    ##   unnest(cols = c(Ma405)) |>
    the_best <- data |>
      distinct(site, sol, proxy, detrend_type, comb, filt_env, Ma405) |>
      nest(.by = c(site, sol, proxy, detrend_type, comb, filt_env)) |>
      mutate(tie_err = NA_real_, RMSD = NA_real_) |>
      assertr::assert(\(x) length(unique(x)) == 1, site, sol, proxy, detrend_type, comb, filt_env) |>
      unnest(data)


    if ("d13C" %in% the_best$proxy) {
      # d13C has a cycle -1, which we can only compute once we have a tie-point for 0
      it <- unique(data$Ma405)[c(2:11, 1)]
    } else {
      it <- unique(data$Ma405)
    }
  ...
  ## bm_0 <- microbenchmark::microbenchmark(times = 1, loop =
      ...
      if (cycle %in% 0:1) {
        ...
        if (cycle == 1 && fix_0) {
          ...
          tmp <- data |>
            ...
            mutate(ecc_sln = approx(slns$age[slns$sln == the_best$sol[[1]]], # TODO: risky!
                                    slns$scl[slns$sln == the_best$sol[[1]]],
                                    xout = age)$y) |>
          ...
          tmp <- data |>
            ...
            mutate(age = pmap_dbl(
                     ...
                     ~ Hmisc::approxExtrap(
                                ...
                                xout = .y)$y)) |>
            # linearly interpolate the astronomical solution eccentricity
            mutate(ecc_sln = approx(slns$age[slns$sln == the_best$sol[[1]]],
                                    slns$scl[slns$sln == the_best$sol[[1]]],
                                    xout = age)$y) |>
            # calculate SD between ecc and ecc_sol
            mutate(SD = (ecc - ecc_sln)^2) #|>
          ...
          smy <- tmp |>
            summarize(RMSD = sqrt(mean(SD)),
                      .by = c(proxy,
                              ...
                              detrend_type,
                              filt_env,
                              tie_0, tie_1, # only for cycles 0 and 1!!!
                              sol)) #|>
        ...
        # this should only occur for d13C
        ## ah this fails, I don't have cycle -1 in agem!
        next
      } else if (cycle > 1) {
    ...
    if (genplot) {
      print(pl ## + plot_layout(title = paste(the_best$sol[[1]],
               ##                             the_best$site[[1]],
               ##                             the_best$proxy[[1]],
               ##                             the_best$detrend_type[[1]],
            ##                             the_best$filt_env[[1]]))
            )
    }
    the_best
  ...
  # example call
  # WARNING: pretty slow!
  tb <- wrap_age_model(subs, agem_sol, fix_0 = FALSE, genplot = TRUE)

  # WARNING: still slow, but slightly less so
  wrap_age_model(subs, agem_sol, fix_0 = TRUE, genplot = TRUE)
  ...
  ## bm_4 <- microbenchmark::microbenchmark(map = wrap_age_model(subs, agem_sol, fix_0 = TRUE),
  ##                                        times = 1)
  ...
  # that means that if all of them take an average of 17 seconds (median value of bm_3)
  ## (nrow(distinct(diffs_tie, proxy, site, detrend_type, comb, filt_env, sol)) *
  (48 *
   summary(bm_3)$median) / 60  # seconds to minutes
  ## / 60 # minutes to hours
  # so 2.84 hours to run all combinations with fix_0 = TRUE.

  # let's first get it to work with some mappage
  the_superduper_best <- diffs_tie |>
    # subset so that we have at least 2 for each (should take about 14 minutes)
    filter(proxy %in% c("d13C", "MS"),
           detrend_type %in% c("scl", "lin_gam_det"),
           comb %in% c("1:1", "0.5:1"),
           sol %in% c("ZB18a", "ZB20a")) |>
    nest(.by = c(proxy, site, detrend_type, comb, filt_env, sol)) |>
    mutate(the_best = pmap(.l = list(dat = data,
                                     prx = proxy,
                                     ste = site,
                                     dty = detrend_type,
                                     cmb = comb,
                                     fen = filt_env,
                                     sln = sol),
                           .f = function(dat, prx, ste, dty, cmb, fen, sln) {
                             dat |>
                               filter(site == ste) |>
                               filter(sol == sln) |>
                               filter(proxy == prx) |>
                               ...
                               filter(comb == cmb) |>
                               filter(filt_env == fen) |>
                               assertr::assert(\(x) length(unique(x)) == 1,
                                               site, proxy, detrend_type, comb, filt_env, sol) #|>
                               ## wrap_age_model(agemodel = agem_sol,
                               ##                tiepoint_uncertainty = seq(-2, 2, 1),
                               ##                fix_0 = TRUE)}
                             }))
...
#+begin_src R
  the_best <- wrap_age_model(subs, agem_sol, fix_0 = TRUE)


  # filter out the best tie-point uncertainties from the age model
  am_tie <- agem_tie |>
    rename(Ma405 = n) |>
    filter(sol == sln) |>
    # we do not include uncertainty on the K/T boundary
    ## tidylog::filter(!(Ma405 == 0 & tie_err != 0)) |>
    tidylog::left_join(the_best,
                       by = c("sol", "site", "Ma405", "tie_err")) |>
    filter(!is.na(RMSD)) |>
    write_csv(glue::glue("out/{sln}_{prx}_best_tiepoint_uncertainty.csv,"))

  # apply the optimized agemodel
  suba <- subs |>
    mutate(age = map_dbl(depth,
             ~ Hmisc::approxExtrap(
                        am_tie |> pull(depth),
                            am_tie |> pull(age),
                        xout = .x)$y)) |>
    mutate(ecc_sln = approx(slns$age[slns$sln == sln],
                            slns$scl[slns$sln == sln],
                            xout = age)$y) |>
    # calculate SD between ecc and ecc_sol
    mutate(SD = (ecc - ecc_sln)^2)
