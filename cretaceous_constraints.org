...
#+begin_src R
  ...
  # create a plot of the tiepoint shifts
  (
  tst1$plot +
    coord_cartesian(xlim = c(150, 0)) +
    geom_vline(xintercept = agem_sol[agem_sol$sol == "ZB18a" & agem_sol$site == "Zumaia", ]$strat_bot)
  ) / (
  tst1$matched |>
  ...
  tst1$matched |>
    ggplot(aes(age * 1e-3, -ecc)) +
    coord_cartesian(xlim = c(69.9, 65.9)) +
    scale_x_reverse("Age (Ma)") +
    geom_vline(xintercept = agem_sol[agem_sol$sol == "ZB18a" & agem_sol$site == "Zumaia", ]$age * 1e-3) +
    geom_line(aes(y = scale(filter)[, 1], colour = sln),
             data = sln_filters |>
               filter(age > 66e3 & age <= 70e3) |>
               filter(target == "405 kyr")) +
