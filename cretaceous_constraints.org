...
#+begin_src R :eval never
  ...
  message("current time: ", lubridate::now())
  rm(wrap_age_model.R) # rm so we can get back the one from the package
#+end_src
...
#+begin_src R :results output graphics file :file imgs/new_rolling_improvement.png :width 3400 :height 1700 :res 300
  ...
  best_age_optim |>
    ...
    facet_grid(rows = vars(proxy), cols = vars(site, comb)) +
    theme(axis.text.x = element_text(angle = 90, size = 7)) +
...
#+begin_src R
  smooth_optim_age_value <- best_age_optim |>
    mutate(isdf = map_lgl(the_best, \(x) is.data.frame(x$matched))) |>
    tidylog::filter(isdf) |>
    tidylog::filter(RMSD == min(RMSD),
...
#+RESULTS:
: filter: removed 126 rows (37%), 217 rows remaining
: filter: removed 28 rows (13%), 189 rows remaining
: filter: removed 182 rows (96%), 7 rows remaining
...
**** plot of matched records and solutions
:PROPERTIES:
...
#+begin_src R :results output graphics file :file imgs/Zumaia_Lstar_1-1_solutions_simple.png :width 2400 :height 2000 :res 300
  best_age_optim |>
    filter(site == "Zumaia", comb == "1:1") |>
    filter(proxy != "d13C") |>
    filter(detrend_type == "lin_scl_fine") |>
    mutate(m = map(the_best, "matched")) |>
    unnest(m) |>
    ggplot(aes(x = age * 1e-3, y = s405 * ecc, colour = proxy)) +
    labs(x = "Age (Ma)", y = "Norm. Filter & Solution") +
    theme(legend.pos = c(.64, .864),
          legend.direction = "horizontal") +
    scale_x_reverse(breaks = seq(80, 50, -1),
    ...
    geom_line() +
    ## annotate("text", x = 66.3, y = 5, label = "RMSD", size = 3) +
    geom_text(aes(x = 66.1, y = 4, label = round(RMSD, 3)),
              size = 2,
              position = position_dodge(width = .7),
              data = \(x) distinct(x, RMSD, site, sol, proxy, detrend_type))
#+end_src
