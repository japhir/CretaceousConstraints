...
#+begin_src R :exports both
  ...
  agem_indep <- agem_simple |>
    mutate(floating_age = n * 405) # in kyr
...
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_mtm.png :width 2600 :height 3200 :res 300
  ...
  specs_fancy |>
    ...
    coord_cartesian(
      ...
      ## ylim = c(2e-5, 0.015) # log
      ) +
    scale_x_continuous(
    ## scale_x_log10(
      sec.axis = sec_axis(trans = ~ 1 / .x,
                                           name = "Period (m)",
                                           breaks = c(50, 10, 5, 3, 2, 1, 0.5))) +
    labs(x = "Frequency (cycles/m)", y = "Spectral power",
...
#+begin_src R
  ...
  mult <- mmm |>
    astrochron::multiTest(flow = 0, fhigh = 0.5) # targeting long and short ecc
...
#+begin_src R
  smooth_ages <- smooth_pred |>
    ...
    mutate(age = Hmisc::approxExtrap(agem_indep$strat_bot,
                                                  agem_indep$floating_age,
                                                  xout = depth)$y,
           .after = depth) |>
    arrange(detrend_type)
#+end_src
...
#+begin_src R :results output graphics file :file imgs/Ma405_categories.png :width 2400 :height 1800 :res 300
  ...
  smooth_ages |>
    ...
    geom_line(aes(x = age * 1e-3 - 65900, y = scl * 2),
              inherit.aes = FALSE,
              data = slns |>
                filter(sln != "La11") |>
                filter(age > 65800) |>
