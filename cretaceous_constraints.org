...
#+RESULTS:
#+begin_example
-/                                                                                                       /                                                                                                       -                                                                                                       > > > . + indexing 18696_2.csv [===============================================================] 1.44GB/s, eta:  0s                                                                                                                                            New names:
• `` -> `...3`
Rows: 629 Columns: 3
── Column specification ─────────────────────────────────────────────────────────────────────────────────
Delimiter: ","
...
:END:
This is a bit of a complex figure. I wanted to get a glance at which proxy for which site scores the best overall MSD.

This shows that the d13C record gives us the overall best MSD's, but the d13C only shows a strong 405 kyr cyclicity, no significant 100 kyr cyclicity (see figure of spectra above).
...
#+begin_src R :results output graphics file :file imgs/Zumaia_MSD-overview.png :width 1920 :height 1080
  msd |>
    ...
    scale_x_reverse(breaks = c(65.6, 66.043, 66.4)) +
    labs(
...
#+caption: *Overview of MSD scores* Assumed age of the K/Pg boundary versus the MSD score. Vertical panels are the sites (Zumaia and the downwards extension in Sopelana) different proxies (MS, L* and d13C). Horizontal panels are the orbital solution the filtered data were compared to (for now only ZB18a and ZB20a), whether I constructed my filtered record as a combination of a direct 405 kyr filter and a 100 kyr filter or as a combination of a 405 kyr filter and the Hilbert transform of the precession filter, and in which proportions I've added the 405 kyr and 100 kyr components together. Vertical lines show the age of the K/Pg boundary with uncertainty based on \ce{^40Ar/^39Ar} and U--Pb data ([cite:@Batenburg2014] cite [cite:@Renne2013] based on new bentonite ages from coals near the K/Pg boundary).
...
#+attr_latex: :width 1.2\linewidth
#+caption: The various assumptions for the K/Pg age on the x-axis versus sedimentation rates on the y-axis. Horizontal panels are grouped by site and proxy, while vertical panels are grouped by orbital solution, filter/Hilbert, and the proportion of the 405 kyr signal compared to the 100 kyr signal.
#+RESULTS:
...
#+begin_src R :results output graphics file :file imgs/Zumaia_ecc_ZB18a.png :width 900
  ...
  diffs |>
    ...
    labs(x = "Age (Ma)",
         ...
         ## colour = "Age of the K/Pg boundary",
...
#+caption: *MSD as a function of the K/Pg age* Here we vary the age of the K/Pg and apply the age model based on 405 kyr cycles identified in the field. The MSD is the mean squared difference between the filtered and scaled proxy record and the filtered and scaled orbital solution.
#+RESULTS:
...
**** COMMENT do the same analysis with different ages for the K/Pg boundary
:PROPERTIES:
