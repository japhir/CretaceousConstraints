...
#+begin_src R :eval never
  ...
  message("duration: ", lubridate::now() - nw)
...
ℹ In argument: `detrend_type = fct_relevel(as_factor(detrend_type),
  names(detrend_colours))`.
Caused by warning:
! 5 unknown levels in `f`: gam_scl, hp_scl, lin_scl_rw, lin_scl_coarse, and
value
2: There was 1 warning in `mutate()`.
ℹ In argument: `comb = fct_relevel(...)`.
Caused by warning:
! 8 unknown levels in `f`: 1:0, 1:0.25, 1:0.5, 1:0.75, 0.75:1, 0.5:1, 0.25:1,
and 0:1
#+end_example
...
#+RESULTS:
: filter: removed 136 rows (56%), 109 rows remaining
...
: window)`
: filter: removed 1,451 rows (1%), 145,186 rows remaining
: + + filter: removed 63 rows (75%), 21 rows remaining
...
: filter: removed 22,442 rows (5%), 408,101 rows remaining
...
#+begin_src R :results output graphics file :file imgs/full_RMSD_scores.pdf :width 5 :height 2
  pl_rmsd_full <- overall_best |>
    ...
    filter(detrend_type == "lin_scl_med") |>
    filter(roll == 1e10) |>
    ## filter(is.na(frac)) |> # we don't want our super narrow 0.1
    filter(proxy == "Lstar") |>
...
#+begin_src R :results output graphics file :file imgs/full_RMSD_roll_comparison.pdf :width 3.3 :height 2
  pl_rmsd_taner <- overall_best |>
    ...
    geom_line(aes(group = "all"), colour = "darkred",
              data = overall_best_old |>
                filter(detrend_type == "lin_scl_med", proxy == "Lstar", window == "gaussian")) +
...
**** simplified comparison of filters
:PROPERTIES:
...
#+begin_src R :results output graphics file :file imgs/full_RMSD_filter_comparison.png :width 2600 :height 700 :res 300
  pl_rmsd_detrend_types <-
    ...
    aes(shape = "taner", linetype = "taner") +
    labs(shape = "", linetype = "") +
    scale_colour_discrete(guide = "none") +
    ## ggnewscale::new_scale_color() +
    ## scale_colour_manual(values = detrend_colours) +
    facet_grid(cols = vars(proxy)) +
    scale_linetype_manual(values = c("taner" = "solid", "gaussian" = "dashed", "rectangular" = "dotted")) +
    geom_line(aes(y = RMSD, ## colour = detrend_type,
                  linetype = window,
                  group = paste(proxy, detrend_type, window)),
              data = overall_best_old |> filter(detrend_type == "lin_scl_med")) +
    scale_shape_manual(values = c("taner" = "circle", "gaussian" = "triangle", "rectangular" = "square")) +
...
**** illustration of improvement in RMSD score
:PROPERTIES:
...
#+begin_src R
  ...
  windows <- windows |>
    mutate(win = map(roll,
                     ...
                     astrochron::linterp(genplot = FALSE, verbose = FALSE) |>
                     astrochron::taner(flow = my_filt_age$flow[[1]], fhigh = my_filt_age$fhigh[[1]],
                                       roll = x,
                                       genplot = FALSE, verbose = FALSE,
                                       output = 2) |>
  ...
  windows <- bind_rows(windows,
    ...
    mutate(win = map(roll,
                     ...
                     simple[, c("age_floating", "value")] |>
                     astrochron::linterp(genplot = FALSE, verbose = FALSE) |>
                     astrochron::taner(flow = my_filt_age$flow[[2]], fhigh = my_filt_age$fhigh[[2]],
                                       roll = x,
                                       genplot = FALSE, verbose = FALSE,
                                       output = 2) |>
                       rename(frequency = V1, amplitude = filter_pos)
  ...
  gauss <-
    bind_rows(
      simple[, c("age_floating", "value")] |>
      astrochron::linterp(genplot = FALSE, verbose = FALSE) |>
      astrochron::bandpass(flow = my_filt_age$flow[[1]], fhigh = my_filt_age$fhigh[[1]],
                           genplot = FALSE, verbose = FALSE,
                           win = 1, output = 2) |>
      ...
      simple[, c("age_floating", "value")] |>
      astrochron::linterp(genplot = FALSE, verbose = FALSE) |>
      astrochron::bandpass(flow = my_filt_age$flow[[2]], fhigh = my_filt_age$fhigh[[2]],
                           genplot = FALSE, verbose = FALSE,
                           win = 1, output = 2) |>
...
#+begin_src R :results output graphics file :file imgs/filter_windows.pdf :width 5 :height 1.7
  pl_windows <- windows |>
    unnest(win) |>
    ...
    labs(x = "Frequency (cycles/kyr)", y = "", colour = "Filter", fill = "Filter") +
    annotate("text", x = 1 / c(405, 110), y = 1,
             label = c("405", "110"), vjust = -0.5) +

    # what if we narrow to 0.1 * freq? TODO
    geom_rect(aes(x = NULL, y = NULL, xmin = flow, xmax = fhigh, ymin = 0, ymax = 1),
               colour = "darkgray",
              data = my_filt_age[2, ] |>
                mutate(range = 0.1 * f, flow = f - range, fhigh = f + range) |>
                mutate(window = "rectangular"),
               alpha = .2) +
     geom_rect(aes(x = NULL, y = NULL, xmin = flow, xmax = fhigh, ymin = 0, ymax = 1),
               colour = "darkgray",
