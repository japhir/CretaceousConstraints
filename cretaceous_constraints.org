...
#+begin_src R
  diffs_tie <- data_ecc |>
    ...
    # I know we want to simulate uncertainty in the agemodel tie-point,
    # but I think it's the same as simulating it in the depth just before applying
    # the age model, right?
    mutate(tie_depth = depth + tie_err) |>
    mutate(tie_age = Hmisc::approxExtrap(agem_simple$strat_bot,
                                         agem_simple$age,
                                         xout = tie_depth)$y) |>
    # add the number of the current 405 kyr cycle to group by (rounded down)
    mutate(Ma405 = findInterval(depth, agem_simple$strat_bot)) |>
