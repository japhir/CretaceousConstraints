...
#+RESULTS:
#+begin_example
-/                                                                                                       /                                                                                                       -                                                                                                       > > > + + + indexing 18696_2.csv [===============================================================] 1.09GB/s, eta:  0s                                                                                                                                            New names:
• `` -> `...3`
Rows: 629 Columns: 3
── Column specification ─────────────────────────────────────────────────────────────────────────────────
Delimiter: ","
...
| sol   |        min(RMSD) |
|-------+------------------|
| ZB20a | 1.00812207545768 |
| ZB18a | 1.03234698602041 |
| La11  | 1.03585551597025 |
...
#+RESULTS:
: filter: removed 108,225 rows (83%), 21,645 rows remaining
...
#+begin_src R :results output graphics file :file imgs/best_fits_age.png :width 1200 :height 800
  ...
  data_diffs_age |>
    ...
    geom_text(aes(y = 0,
                  group = site,
                  label = paste(proxy_lag, detrend_type, comb, filt_env)),
              hjust = 0,
              data = data_diffs_age |>
                inner_join(best_age) |>
                summarize(age = max(age),
                          .by = c(sol, site, proxy,
                                  proxy_lag, detrend_type, comb, filt_env))) +
    scale_x_reverse(breaks = 66:72, minor_breaks = seq(59.9, 73, .1)) +
    ## scale_colour_viridis_d() +
    scale_colour_brewer(type = "qual", guide = FALSE) +
    scale_alpha_continuous(range = c(1, .3)) +
