...
#+begin_src R :exports none :results none
  smooth_all <- smooth_basic |>
    ...
    ## bind_rows(smooth_hp) |>
    pivot_longer(c(value, scl, lin, lin_gam, lin_fine, lin_coarse, lin_scl, lin_scl_fine, lin_scl_coarse, gam_scl, lin_pred, lin_pred_fine, lin_pred_coarse, lin_gam_pred, lin_gam_det),
  ...
  smooth_pred <- smooth_all |>
    tidylog::filter(detrend_type %in% c("value", "scl",
                               "lin_scl", "lin_scl_fine", "lin_scl_coarse",
                               "gam_scl"#, ## "lin_gam_det"
                               ))
...
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_records_complicated.png :height 1200 :width 2600 :res 300
  ...
  smooth |>
    ...
    geom_line(aes(y = lin_pred, colour = "lin", group = colour),
              ...
              data = smooth |>
                filter(site == "Zumaia", proxy == prx)) +
    geom_line(aes(y = lin_pred, colour = "lin_2", group = colour),
              linewidth = 1.2,alpha = .6,
              data = smooth_2 |>
                filter(site == "Zumaia", proxy == prx)) +
    geom_line(aes(y = lin_pred, colour = "lin_3", group = colour),
              linewidth = 1.2,alpha = .6,
              data = smooth_3 |>
                filter(site == "Zumaia", proxy == prx)) +
