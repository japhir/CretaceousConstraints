...
** general stuff needed for all analyses
:PROPERTIES:
:CREATED:  [2023-10-03 Tue 16:31]
:END:
*** Orbital Solutions
:PROPERTIES:
...
#+begin_src R
  ...
  sln <- ZB18a
    ## bandpass_filter(freqs = my_os_freqs,
    ##                 x = age, y = ecc,
    ##                 nest = "sln") |>
    ## select(-p, -f, -fr, -flow, -fhigh) |>
    ## pivot_wider(names_from = target, values_from = filt) |>
    ## mutate(filt = scale(`405 kyr` + `100 kyr`)[, 1])
#+end_src
...
:END:
these depend on the AS you choose!
#+begin_src R
  kpg_ages <- tribble(
    ~ sln, ~ age1, ~ age2,
    "ZB18a", 65.94, 65.96,
...
:END:
#+begin_src R :results output graphics file :file imgs/orbital_solutions.png :width 900
  sln |>
    bind_rows(sln2) |>
    bind_rows(sln3) |>
...
#+RESULTS:
: indexing ODP208_1267_MS.csv [==================================================----] 282.81GB/s, eta:  0sindexing ODP208_1267_MS.csv [========================================================] 2.85GB/s, eta:  0s                                                                                                                                            Rows: 23239 Columns: 28
: ── Column specification ─────────────────────────────────────────────────────────────────────────────────
: Delimiter: ","
...
#+RESULTS:
#+begin_example
-/                                                                                                       /                                                                                                       -                                                                                                       > > > + + + indexing 18696_2.csv [===============================================================] 1.78GB/s, eta:  0s                                                                                                                                            New names:
• `` -> `...3`
Rows: 629 Columns: 3
── Column specification ─────────────────────────────────────────────────────────────────────────────────
Delimiter: ","
...
#+begin_src R :exports none
  ...
  agem_simple <- agem |>
    ...
    mutate(age = case_when(sol == "ZB18a" ~
                             sln_peaks |>
                             filter(sln == "ZB18a", age <= 66.0 * 1e3) |>
                             arrange(rev(age)) |>
                             slice(n + 1) |>
                             pull(age),
                           sol == "ZB20a" ~
                             sln_peaks |>
                             filter(sln == "ZB20a", age <= 65.9 * 1e3) |>
                             arrange(rev(age)) |>
                             slice(n + 1) |>
                             pull(age),
                           sol == "La11" ~
                             sln_peaks |>
                             filter(sln == "La11", age <= 66.0 * 1e3) |>
                             arrange(rev(age)) |>
                             slice(n + 1) |>
                             pull(age),
                           TRUE ~ NA_real_)) |>
...
#+RESULTS:
: indexing 405_minima.csv [==========================================================] 214.85MB/s, eta:  0s                                                                                                                                            Rows: 20 Columns: 5
: ── Column specification ─────────────────────────────────────────────────────────────────────────────────
: Delimiter: ","
...
#+caption: Zumaia/Sopelana spectral analysis in the depth domain.
#+RESULTS:
...
#+begin_src R
  filts_basic <- comp |>
    bandpass_filter(freqs = my_filters, x = depth, y = value, nest = c("proxy", "site"))
#+end_src
...
#+begin_src R
  filts_scl <- comp |>
    mutate(scl = scale(value)[, 1], .by = c(site, proxy)) |>
    bandpass_filter(freqs = my_filters, x = depth, y = scl, nest = c("proxy", "site"))
#+end_src
...
:END:
#+begin_src R
  filts_gam <- smooth_basic |>
    bandpass_filter(freqs = my_filters, x = depth, y = det, nest = c("proxy", "site"))
#+end_src
...
#+begin_src R :exports none
  filts_hil <- filts_fancy |>
    ...
    nest(.by = c(proxy, site, target, p, f, range, flow, fhigh, ref)) |>
    mutate(
      hil = map(data, \(d) d |>
                           select(depth, filt) |>
...
:END:
:LOGBOOK:
- State "SOME"       from "DONE"       [2023-10-03 Tue 13:57]
- State "DONE"       from "SOME"       [2023-10-03 Tue 13:57]
...
#+caption: Zumaia/Sopelana simple gam trend removal and filtering.
#+RESULTS:
...
#+begin_src R :results none
  data_ecc <-
    ...
    tidylog::left_join(filts_hil |>
                       select(-target, -p, -range, -f, -flow, -fhigh, -ref)) |>
    mutate(
...
#+begin_src R :results output graphics file :file imgs/Zumaia_ecc_depth.png :width 900
  data_ecc |>
    ...
    geom_line() +
    geom_point(aes(x = strat_bot, y = -2), inherit.aes = FALSE, data = agem_simple) +
    labs(x = "Stratigraphic height (m)", y = "Normalized scaled combination of filters",
         colour = "Method to generate ecc")
#+end_src

#+caption: Zumaia/Sopelana artificial combinations of 405 kyr and 100 kyr bandpass filtered records in various combinations (colour).
#+RESULTS:
  ...
  diffs <- data_ecc |>
    ...
    mutate(ecc_sln = case_when(
             ...
             sol == "La11" ~ approx(sln3$age, sln3$scl,
                                    xout = age)$y,
             TRUE ~ NA_real_)) |>
    # make sure we put in the phase relationship correctly
    # TODO: decide whether to do the proxy phase relationship shift here or in raw ecc
    mutate(SD = (ecc_sln - ecc)^2)

  msd <- diffs |>
    summarize(RMSD = sqrt(mean(SD)),
              .by = c(proxy, site, comb, filt_env, inv_sedrate,
                      kpg_age, proxy_lag, sol)) |>
    arrange(RMSD)
#+end_src

#+RESULTS:
: filter: removed 21,645 rows (83%), 4,329 rows remaining

**** plot RMSD directly
:PROPERTIES:
:CREATED:  [2023-10-02 Mon 10:38]
:END:
This is a bit of a complex figure. I wanted to get a glance at which proxy for which site scores the best overall RMSD.

This shows that the d13C record gives us the overall best RMSD's, but the d13C only shows a strong 405 kyr cyclicity, no significant 100 kyr cyclicity (see figure of spectra above).
...
#+attr_latex: :width 1.2\linewidth
#+caption: The various assumptions for the K/T age on the x-axis versus sedimentation rates on the y-axis. Horizontal panels are grouped by site and proxy, while vertical panels are grouped by orbital solution, filter/Hilbert, and the proportion of the 405 kyr signal compared to the 100 kyr signal.
#+RESULTS:
[[file:imgs/Zumaia_ecc_RMSD.png]]

#+caption: Older figure, higher resolution in both \tau and age_kpg, but without La11 solution.
...
#+RESULTS:
| proxy | site   |  comb | filt_env | inv_sedrate | kpg_age | proxy_lag | sol   |             RMSD |
|-------+--------+-------+----------+-------------+---------+-----------+-------+------------------|
| d13C  | Zumaia | 0.5:1 | filter   |          29 |   66000 |        59 | La11  |  1.2580965317243 |
| d13C  | Zumaia | 0.5:1 | filter   |          29 |   66000 |        54 | La11  | 1.26117245520631 |
| d13C  | Zumaia | 0.5:1 | filter   |          29 |   66000 |        49 | La11  | 1.26456999609458 |
| Lstar | Zumaia | 0.5:1 | filter   |          24 |   65960 |        15 | La11  | 1.38360530818251 |
| Lstar | Zumaia | 0.5:1 | filter   |          28 |   65920 |        20 | La11  | 1.38566138000714 |
| Lstar | Zumaia | 0.5:1 | filter   |          28 |   65940 |         0 | La11  | 1.38566138000714 |
| d13C  | Zumaia | 0.5:1 | filter   |          27 |   66020 |        54 | ZB18a | 1.41361658411153 |
| d13C  | Zumaia | 0.5:1 | filter   |          27 |   66000 |        64 | ZB20a | 1.41785782896381 |
| d13C  | Zumaia | 0.5:1 | filter   |          27 |   66020 |        44 | ZB20a | 1.41785782896381 |
| d13C  | Zumaia | 0.5:1 | filter   |          27 |   66020 |        59 | ZB18a | 1.42638306016871 |
| MS    | Zumaia | 0.5:1 | filter   |        30.5 |   66000 |        20 | La11  | 1.42747632874423 |
| MS    | Zumaia | 0.5:1 | filter   |        30.5 |   66020 |         0 | La11  | 1.42747632874423 |
| MS    | Zumaia | 0.5:1 | filter   |        30.5 |   66020 |         5 | La11  | 1.42933806380579 |
| d13C  | Zumaia | 0.5:1 | filter   |          27 |   66020 |        64 | ZB18a | 1.43270452580087 |
| d13C  | Zumaia | 0.5:1 | filter   |        26.5 |   66020 |        64 | ZB20a |    1.43301545589 |
| Lstar | Zumaia | 0.5:1 | filter   |        29.5 |   65940 |         5 | ZB20a | 1.44898855638788 |
| Lstar | Zumaia | 0.5:1 | filter   |        29.5 |   65920 |        20 | ZB20a | 1.45484116997998 |
| Lstar | Zumaia | 0.5:1 | filter   |        29.5 |   65940 |         0 | ZB20a | 1.45484116997998 |
| MS    | Zumaia | 0.5:1 | filter   |        26.5 |   66000 |         5 | ZB20a | 1.48978477832179 |
| MS    | Zumaia | 0.5:1 | filter   |        26.5 |   65980 |        20 | ZB20a | 1.49178400833267 |
| MS    | Zumaia | 0.5:1 | filter   |        26.5 |   66000 |         0 | ZB20a | 1.49178400833267 |
| Lstar | Zumaia | 0.5:1 | filter   |        29.5 |   65980 |         5 | ZB18a | 1.51500591628103 |
| Lstar | Zumaia | 0.5:1 | filter   |        29.5 |   65960 |        20 | ZB18a | 1.51705821919288 |
| Lstar | Zumaia | 0.5:1 | filter   |        29.5 |   65980 |         0 | ZB18a | 1.51705821919288 |
| MS    | Zumaia | 0.5:1 | filter   |          29 |   66000 |        20 | ZB18a | 1.55139033048297 |
| MS    | Zumaia | 0.5:1 | filter   |          29 |   66020 |         0 | ZB18a | 1.55139033048297 |
| MS    | Zumaia | 0.5:1 | filter   |          29 |   66020 |        10 | ZB18a | 1.55212535889634 |
| MS    | Zumaia | 0.5:1 | filter   |          29 |   66020 |         5 | ZB18a | 1.55795659052211 |

#+caption: Zumaia non-d13C records with the lowest RMSD score per OS.

Or just blindly calculate the average RMSD of all the different approaches.

#+begin_src R :results value :colnames yes
  msd |>
    filter(site == "Zumaia") |>
    ## filter(proxy != "d13C") |>
    summarize(min(RMSD), .by = sol)
#+end_src

#+caption: Zumaia non-d13C records average RMSD score per OS.
#+RESULTS:
| sol   |         min(RMSD) |
|-------+-------------------|
| ZB20a | 0.930825647668838 |
| La11  | 0.949297128969555 |
| ZB18a | 0.977435332898786 |
...
#+caption: Zumaia best MS record match with ZB20a (lowest RMSD score).
#+attr_latex: :width \linewidth
#+RESULTS:
[[file:imgs/Zumaia_ecc_ZB18a.png]]

*** COMMENT Analysis in the time domain
:PROPERTIES:
:CREATED:  [2023-09-29 Fri 13:11]
:END:
**** read in the age model from the field
:PROPERTIES:
:CREATED:  [2023-09-25 Mon 11:16]
:END:
[cite:@Batenburg2012] identified the 405 kyr minima in the field, and linked them to their MS and L* records. Each cycle is assigned an age of exactly 405 kyr.
...
#+begin_src R
  ...
  specs_age_gam <- smooth |>
    spectral_analysis(nest = c("proxy", "site"), x = age, y = gam)

  specs_age_raw <- smooth |>
    spectral_analysis(nest = c("proxy", "site"), x = age, y = value)
#+end_src
...
:END:
The d13C, L*, and MS records only show significant spectral peaks around the 100 kyr cycle if I do my partial linear detrending first.

#+begin_src R :results output graphics file :file imgs/Zumaia_Sopelana_spectra_filters.png :width 900 :height 800
  # plot spectra w/ Freq on x-axis
  specs_age |>
    mutate(method = "linear detrend") |>
    bind_rows(
      specs_age_raw |>
      mutate(method = "raw value")
    ) |>
    bind_rows(
      specs_age_gam |>
      mutate(method = "gam")
    ) |>
    ggplot(aes(x = freq, y = power, colour = method)) +
    facet_grid(#cols = vars(site),
               rows = vars(site, proxy)) +
    geom_line() +
    geom_ribbon(aes(ymin = AR1_fit, ymax = AR1_power,
                    linetype = NA,
                    fill = method,
                    group = paste(method, site, .width)),
                alpha = .1) +
    ...
    geom_rect(aes(xmin = flow, xmax = fhigh, ymin = 1e-5, ymax = 1, fill = ref),
              inherit.aes = FALSE,
              alpha = .3, data = my_filt_age) +
    labs(x = "Frequency (cycles/kyr)", y = "Power")
#+end_src

#+caption: Zumaia/Sopelana spectral analysis after applying the 405 kyr tie-point age model from the field by [cite:@Batenburg2012].
#+RESULTS:
...
#+begin_src R :results none
  filts_age <- smooth |>
    bandpass_filter(freqs = my_filt_age, x = age, y = det, nest = c("proxy", "site"))
#+end_src
...
:END:
#+begin_src R :results none
  hb_age_prec <- filts_age |>
    filter(target == "prec") |>
    nest(.by = c(site, proxy)) |>
    mutate(hb = map(data, \(d) d |>
                               select(age, filt) |>
                               astrochron::hilbert(genplot = FALSE, verbose = FALSE))) |>
    unnest(hb) |>
...
#+begin_src R :results output graphics file :file imgs/Zumaia_Sopelana_records_and_filters.png :width 900
  smooth |>
    ...
    geom_line(aes(y = det), alpha = .6) +
    geom_line(aes(y = filt + as.integer(as.factor(target)), colour = target), data = filts_age) +
    geom_line(aes(y = envelope + 3.5, group = site), colour = "red", data = hb_age_prec) +
    geom_point(aes(y = pk + 1.5),
               ...
               data = filts_age |>
                 ...
                 mutate(pk = -pk)) +
    scale_x_reverse(breaks = seq(0, 5, 1)) +
    labs(x = "K/T + Age (Ma)", y = "Normalized proxy",
         ## title = "Zumaia raw proxy record filters"
         ## title = "Zumaia fancy detrended proxy record filters"
         )
#+end_src

#+caption: Zumaia/Sopelana detrended record and various filters. Red dots identify minima in the 405 kyr filter, which should correspond to the vertical black lines (minima identified in the field) for MS and to the maxima for L*. d13C shows a lag with respect to the other proxies.
#+RESULTS:
[[file:imgs/Zumaia_Sopelana_records_and_filters.png]]

**** add the orbital solution to the ecc curve and plot the RMSD
:PROPERTIES:
:CREATED:  [2023-09-26 Tue 16:51]
:END:
#+begin_src R :results none
  ## kpg_age <- 65970 # age from Batenburg et al., 2014, filtered La11 solution
  kpg_age <- 66043 # age from Renne et al., 2013

  data_ecc <- filts_age |>
    # add the hilbert precession envelope to the data
    left_join(
      hb_age_prec |>
      select(proxy, site, age, envelope)
    ) |>
    select(proxy, site, target, age, filt, envelope) |>
    pivot_wider(names_from = target, values_from = filt) |>
    mutate(
      fracs = list(tribble( ~ comb, ~ s405, ~ s100,
                          "1:1", 1, 1,
  ...
  data_diffs <- data_ecc |>
    # because we don't vary the sedimentation rate here but use the field-based
    # age model, we can vary the kpg_age a bit more incrementally
    mutate(kpg_age = list(seq(65500, 66500, 10))) |>
    unnest(cols = c(kpg_age)) |>
    mutate(age = age + kpg_age) |>
    mutate(sol = list(c("ZB18a", "ZB20a"))) |>
    unnest(cols = c(sol)) |>
    mutate(ecc_sln = case_when(
             sol == "ZB18a" ~ approx(sln$age, sln$filt,
                                     xout = age)$y,
             sol == "ZB20a" ~ approx(sln2$age, sln2$filt,
                                     xout = age)$y,
             TRUE ~ NA_real_),
           squared_diff = (ecc_sln - ecc)^2) |>
    select(-envelope, -`405 kyr`, -`100 kyr`)

  msd_age <- data_diffs |>
      summarize(RMSD = sqrt(sum(squared_diff)),
                .by = c(proxy, site, comb, filt_env, kpg_age, sol)) |>
      arrange(RMSD)
#+end_src
...
#+begin_src R :results output graphics file :file imgs/Zumaia_MSD-vs-age.png :width 900
  data_ecc |>
    ggplot(aes(x = age * 0.001, y = ecc, group = site)) +
    facet_grid(rows = vars(proxy),
...
#+begin_src R :results output graphics file :file imgs/Zumaia_age_MSD-overview.png :width 800
  msd_age |>
    ...
    geom_vline(xintercept = c(## kpg_age * 0.001, # based on 405 kyr minima in La11
               ...
               alpha = .3) +
    geom_line(aes(group = paste(comb, filt_env, site, proxy)), linewidth = .7
              ) +
    scale_colour_viridis_d() +
    scale_x_reverse(breaks = c(65.6, 66.0,#66.043,
                               66.4)) +
    labs(
...
#+caption: *RMSD as a function of the K/T age* Here we vary the age of the K/T and apply the age model based on 405 kyr cycles identified in the field. The RMSD is the mean squared difference between the filtered and scaled proxy record and the filtered and scaled orbital solution.
#+RESULTS:
...
**** plot the curves with the lowest overall RMSD
:PROPERTIES:
:CREATED:  [2023-10-02 Mon 16:12]
:END:
#+begin_src R :results output graphics file :file imgs/Zumaia_ecc_age_example.png :width 900
  data_diffs |>
    # the "best" Zumaia setup?
    filter(
    (site == "Zumaia" & proxy == "Lstar" &
      comb == "1:0.5" & filt_env == "filter" &
      kpg_age == 66070 & sol == "ZB18a") |
    (site == "Zumaia" & proxy == "MS" &
      comb == "0.5:1" & filt_env == "filter" &
      kpg_age == 66290 & sol == "ZB20a")) |>
    # filter out some of the best fits
    left_join(
      msd_age |>
      ## filter(proxy != "d13C") |>
    ...
    facet_grid(rows = vars(proxy, sol),
               ...
               scales = "free_x", space = "free_x") +
    geom_line(aes(y = ecc_sln,
                  colour = paste(sol, "1:1 405:100 kyr filter")),
              linetype = 1, linewidth = 1.2) +
    # plot the raw orbital solution
    ## geom_line(aes(x = age * 0.001, y = scl),
    ##           alpha = .2,
    ##           inherit.aes = FALSE, colour = "purple", data = ZB20a) +
    # plot the 405 kyr filter of the solution
    geom_line(aes(x = age / 1000,
                  y = scale(`405 kyr`)[,1],
                  colour = paste(sol, "405 kyr filter")),
              inherit.aes = FALSE,
              data = sln2 |>
                mutate(proxy = "MS") |>
                bind_rows(sln |>
                          mutate(proxy = "Lstar")) |>
                rename(sol = sln)) +
    ## geom_line(aes(x = age / 1000, y = scale(`100 kyr`)[,1]), inherit.aes = FALSE,
    ##           colour = "purple", data = sln2) +
    geom_line(aes(y = ecc, MSD = MSD,
                  colour = paste(kpg_age / 1000)), #paste(comb, filt_env)),
                  linewidth = 1.2) +
    ## gghighlight::gghighlight(MSD < 23) + # not a fan of this
    ## scale_colour_viridis_c() +
    ## scale_alpha_continuous(range = c(.9, .05)) +
    ...
    ## geom_line(sln)
    labs(x = "Age (Ma)",
         y = "Scaled proxy/solution filter",
         linetype = "405:100 kyr weighing",
         ## colour = "Age of the K/T boundary",
         ## colour = "Sedimentation rate (cm/kyr)",
         colour = "",
         alpha = "RMSD\n(lower = better)",
         title = "Example comparison of Zumaia MS after applying 405-kyr tuned age model from the field (Batenburg et al., 2016)")
#+end_src

#+caption: Best fit (lowest RMSD) for Zumaia/Sopelana based on the 405-kyr age model from the field.
#+attr_latex: :width \linewidth
#+RESULTS:
[[file:imgs/Zumaia_ecc_age_example.png]]
...
:END:
#+begin_src R
  agem_tie <- agem_simple |>
    mutate(tie_err = list(seq(-2, 2, 1))) |>
    unnest(cols = c(tie_err)) |>
    mutate(strat_adj = strat_bot + tie_err) |>
    arrange(tie_err, strat_adj) |>
    mutate(depth_diff = strat_adj - lag(strat_adj), .by = tie_err) |>
    mutate(tau_calc = 405 / depth_diff)
    ## filter(!(strat_bot == 0 & tie_err != 0))
  # aargh this is not doing what I want!? it'll just pick ONE uncertainty and apply it to each point
  # what I want to do is check for Ma405 = 0 what the best tie_err is, then for Ma405 = 1, etc.
  ...
  msd_tie <- diffs_tie |>
    summarize(RMSD = sqrt(mean(SD)),
              .by = c(proxy, site, comb, filt_env, tau,
                      Ma405,
                      tie_err, kpg_age, proxy_lag, sol)) |>
    arrange(RMSD, Ma405)
...
#+begin_src R :results none
  best_tie <- msd_tie |>
    ...
    filter(
      RMSD == min(RMSD), .by = c(Ma405, kpg_age, proxy_lag)
    ) |>
    arrange(Ma405)
...
**** plot the curve with the lowest overall RMSD
:PROPERTIES:
:CREATED:  [2023-09-27 Wed 19:03]
:END:
#+begin_src R :results output graphics file :file imgs/best_fits_tiepoints.png :width 900
  # this plot crashes if I plot them all
  diffs_tie |>
    ...
    ggplot(aes(x = age * 0.001,
               group = paste(comb, filt_env, site, kpg_age, proxy_lag),
               ## colour = 1 / tau * 100, # convert from kyr/m to cm/kyr
               ## colour = kpg_age,
               # filt_env??
               )) +
    ...
    geom_line(aes(y = ecc, RMSD = RMSD,
              ...
              linewidth = 1.2) +
    ## geom_point(aes(x = tie_age + kpg_age)
    scale_x_reverse() +
    scale_colour_viridis_d() +
    geom_vline(aes(xintercept = kpg_age * 0.001), colour = "red") +
    labs(x = "Age (Ma)",
...
* COMMENT paste this in the biblatex call after export to tex
:PROPERTIES:
:CREATED:  [2023-10-02 Mon 14:53]
:END:
