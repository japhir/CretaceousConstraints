...
manually converted to csv again
...
#+begin_src R
  rmcd <- readr::read_csv("dat/ODP208_1267_rmcd.csv") |>
    mutate(hole = str_sub(label, start = 5, end = 5),
           hole2 = str_sub(label2, start = 5, end = 5)) |>
    mutate(row = 1:n())

  rmcd |>
    ggplot(aes(x = depth_mbsf, y = row)) +
    geom_point(color = "blue", alpha = .2) +
    geom_point(aes(x = depth_mbsf2), color = "red", alpha = .2) +
    geom_segment(aes(xend = depth_mbsf2, yend = row),
