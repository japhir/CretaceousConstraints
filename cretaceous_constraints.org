  ...
  rolling_age_optim_old <-
    ...
    bind_rows(
      bind_rows(
        ...
        read_rds("out/2024-05-08_rolling_age_optim_Lstar-d13C-MS_lin-scl-med_gauss_0.25_tie-error-1.6_slide-300_alpha5.rds") |> mutate(frac = 0.25 * 2.1227 / 3 * 5) |> mutate(date = ymd("2024-05-08"), .before = "proxy"), # notice no gauss corr b/c I don't know how to do this
      ) |>
...
#+begin_src R
  ...
  overall_best_old <- overall_sd_old |>
    summarize(RMSD = sqrt(mean(SD)),
              .by = c(date, proxy, detrend_type, comb, sol, window, frac))
...
: filter: removed 27,583 rows (4%), 594,065 rows remaining
