...
**** inspect the tiepoint optimization plots
:PROPERTIES:
...
#+begin_src R
  ...
  sopelana_ids <- c(8, 22, 36, 50, 64, 78)
  zumaia_ids <- c(1, 15, 29, 43, 57, 71)
...
#+RESULTS:
: Joining with `by = join_by(proxy, detrend_type, depth_chunk, comb, sol, max_age_slider, tiepoint_error, frac, window)`
: filter: removed 1,179 rows (2%), 46,321 rows remaining
: + + filter: removed 16 rows (57%), 12 rows remaining
