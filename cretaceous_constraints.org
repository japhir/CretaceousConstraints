...
Ehhm... there are no spectral peaks in the range that they filter!

**** NEXT remake this figure using ggplot marking the region that they taner filter
:PROPERTIES:
:CREATED:  [2023-09-27 Wed 19:25]
:END:
**** do wavelet analysis
:PROPERTIES:
...
#+begin_src R :results none
  filts_hil <- filts_fancy |>
    ...
    nest(.by = c(name, site, target, center, f, range, flow, fhigh, ref)) |>
    mutate(
      hil = map(data, \(d) d |>
                           select(depth, filt) |>
...
#+RESULTS:
**** NEXT now filter out the 405 kyr and 100 kyr of the solution and generate a curve from that!
:PROPERTIES:
:CREATED:  [2023-09-27 Wed 19:19]
:END:
...
#+begin_src R :results none
  data_ecc <-
    ...
    pivot_wider(id_cols = c(name, site, depth),
    ...
    arrange(rev(site), rev(name), filt_env, comb, depth)
#+end_src
...
#+begin_src R
  ...
  diffs <- data_ecc |>
    ...
    mutate(ecc_sln = case_when(
             sol == "ZB18a" ~ approx(sln$age, sln$scl, xout = age)$y,
             sol == "ZB20a" ~ approx(sln2$age, sln2$scl, xout = age)$y,
             TRUE ~ NA_real_),
  ...
  msd <- diffs |>
    summarize(MSD = sqrt(sum(squared_diff)),
              .by = c(name, site, comb, filt_env, tau, kpg_age, sol)) |>
    arrange(MSD)
#+end_src
...
#+begin_src R :results output graphics file :file imgs/Zumaia_ecc_ZB18a.png :width 900
  ...
  diffs |>
    tidylog::filter(comb == "0.5:1") |> # to ~5 M
    tidylog::filter(filt_env == "hilbert") |> # to ~2.4 M
    tidylog::filter(site == "Zumaia") |>
    # I first didn't filter to find the ones with the lowest MSD
    ## summarize(MSD = sqrt(sum(diff)), .by = c(name, site, comb, filt_env, tau, kpg_age, sol)) |>
    ## arrange(MSD)
    tidylog::filter((kpg_age == 66200 & sol == "ZB20a") |
                    (kpg_age == 65800 & sol == "ZB18a")) |>
    tidylog::filter(tau == 26) |>
    ggplot(aes(x = age, group = paste(comb, filt_env, site, tau, kpg_age),
               ## colour = tau,
               ## colour = kpg_age,
               )) +
    facet_grid(rows = vars(name), cols = vars(sol)) +
    geom_line(aes(y = ecc)) +
...
:END:
#+begin_src R :results output graphics file :file imgs/Zumaia_ecc_MSD.png :width 1920 :height 1080
  msd |>
    ...
    facet_grid(rows = vars(name), cols = vars(sol, filt_env, comb)) +
    ## geom_line() +
    geom_raster() +
...
#+begin_src R :results output graphics file :file imgs/Zumaia_time-slider.png :width 900
  data_ecc_off_summ |>
    ggplot(aes(x = age_kpg * 0.001, y = value, colour = name, group = paste(site, name))) +
    geom_line() +
