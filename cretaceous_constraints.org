...
#+RESULTS:
#+begin_example
-/                                                                                                       /                                                                                                       -                                                                                                       > > > + + + indexing 18696_2.csv [===============================================================] 1.47GB/s, eta:  0s                                                                                                                                            New names:
• `` -> `...3`
Rows: 629 Columns: 3
── Column specification ─────────────────────────────────────────────────────────────────────────────────
Delimiter: ","
...
#+begin_src R :results output graphics file :file imgs/orbital_solutions.png :width 900
  sln |>
    bind_rows(sln2) |>
    bind_rows(sln3) |>
    ggplot(aes(x = age / 1000, y = ecc, colour = sln)) +
    geom_line(linewidth = 1.2) +
    coord_cartesian(xlim = c(69, 65)) +
    labs(x = "Age (Ma)", y = "Eccentricity (-)", colour = "OS")
#+end_src
...
#+RESULTS:
: indexing 405_minima.csv [==========================================================] 243.98MB/s, eta:  0s                                                                                                                                            Rows: 20 Columns: 5
: ── Column specification ─────────────────────────────────────────────────────────────────────────────────
: Delimiter: ","
...
#+begin_src R :results output graphics file :file imgs/Zumaia-Sopelana_records_simple.png :width 900
  comp |>
    ...
    ## geom_segment(aes(x = strat_bot - 4, xend = strat_bot + 4,
    ##                  y = -2, yend = -2),
    ##              inherit.aes = FALSE,
    ##              data = agem_simple,
    ##              linewidth = 2) +
...
**** add the orbital solution to the data and calculate the RMSD
:PROPERTIES:
...
#+begin_src R
  ...
  # 13 405 kyr cycles in 183 m of sediment, kyr/m
...
#+begin_src R :results output graphics file :file imgs/Zumaia_MSD-overview.png :width 1920 :height 1080
  msd |>
    ## filter(comb == "1:1", filt_env == "filter") |>
    ...
    geom_vline(xintercept = c(## kpg_age * 0.001, # based on 405 kyr minima in La11
               ...
               alpha = .3) +
    geom_line(aes(group = paste(tau, comb, filt_env)), linewidth = .7
              ) +
...
#+begin_src R :results output graphics file :file imgs/Zumaia_ecc_ZB18a.png :width 900
  ...
  diffs |>
    ## filter(kpg_age == 66100) |> # lowest global msd?
    ## filter(proxy != "d13C") |>
    # the "best" Zumaia setup?
    filter(
    (site == "Zumaia" & proxy == "MS" &
      comb == "0.5:1" & filt_env == "filter" &
      tau == 26 & kpg_age == 65900 & sol == "ZB20a") |
    (site == "Zumaia" & proxy == "MS" &
      comb == "0.5:1" & filt_env == "filter" &
      tau == 22 & kpg_age == 66150 & sol == "ZB20a")) |>
    ...
    ggplot(aes(x = age * 0.001,
               group = paste(comb, filt_env, site, tau, kpg_age),
               ## alpha = MSD,
    ...
    geom_line(aes(x = age / 1000,
                  y = scale(`405 kyr`)[,1],
                  colour = paste(sln, "405 kyr filter")),
              inherit.aes = FALSE, data = sln2) +
    ## geom_line(aes(x = age / 1000, y = scale(`100 kyr`)[,1]), inherit.aes = FALSE,
    ##           colour = "purple", data = sln2) +
    geom_line(aes(y = ecc, MSD = MSD,
                  colour = paste(kpg_age / 1000, 1 / tau * 100)), #paste(comb, filt_env)),
                  linewidth = 1.2) +
    ...
    labs(x = "Age (Ma)",
         ...
         colour = "",
         alpha = "RMSD\n(lower = better)",
         title = "Example comparison of Zumaia MS")
#+end_src
...
#+begin_src R :results value :colnames yes
  msd |>
    ...
    mutate(low = min(RMSD),
              .by = c(## proxy, site,
                      sol)) |>
    # have a closer look at that one
    filter(low == RMSD) |>
    arrange(RMSD) |>
    select(-low)
#+end_src
...
#+RESULTS:
| proxy | site   |  comb | filt_env | tau | kpg_age | sol   |             RMSD |
|-------+--------+-------+----------+-----+---------+-------+------------------|
| Lstar | Zumaia | 1:0.5 | filter   |  25 |   65740 | ZB18a | 1.02265982866026 |
| Lstar | Zumaia | 1:0.5 | filter   |  24 |   65800 | ZB20a | 1.07609327533125 |
| Lstar | Zumaia | 1:0.5 | filter   |  24 |   65500 | La11  | 1.07683523869387 |
...
| sol   |       mean(RMSD) |
|-------+------------------|
| ZB18a | 1.43064454271371 |
| ZB20a |  1.4257944763742 |
| La11  | 1.42441067205021 |
...
#+begin_src R
  ...
  mutate(Ma405 = Hmisc::approxExtrap(agem_simple$strat_bot, agem_simple$n,
                                     xout = depth)$y |>
                                                  floor()) |>
  nest(.by = Ma405) |>
  mutate(tiepoint_uncertainty = list(seq(-4, 4, 1))) |>
  unnest(cols = c(tiepoint_uncertainty)) |> # tiepoint uncertainty in metres
  unnest(cols = c(data)) |> # tiepoint uncertainty in metres
  mutate(age = kpg_age +
           Hmisc::approxExtrap(agem_simple$strat_bot + tiepoint_uncertainty,
                               agem_simple$age,
                               xout = depth)$y) |>
#+end_src
** Gubbio
