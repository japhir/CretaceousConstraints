...
#+begin_src R
  rolling_age_optim <-
    bind_rows(
      ...
      read_rds("out/2024-03-21_rolling_age_optim_Lstar_scl_taner.rds")
    ) |>
    mutate(detrend_type = as_factor(detrend_type) |>
             fct_relevel(names(detrend_colours))) |>
    mutate(comb = as_factor(comb) |>
...
Caused by warning:
! 6 unknown levels in `f`: gam_scl, hp_scl, lin_scl_rw, lin_scl_fine,
lin_scl_coarse, and value
...
: filter: removed 456 rows (1%), 38,105 rows remaining
...
**** recompute RMSD for the raw taner filter
:PROPERTIES:
...
#+begin_src R :results output graphics file :file imgs/full_RMSD_scores.pdf :width 5 :height 2
  pl_rmsd_full <- overall_best |>
    ...
    ##           alpha = .1) +
    ## geom_point(alpha = .05) +
    geom_line(aes(group = paste(detrend_type, proxy)),
                  alpha = .3) +
    geom_point() +
    theme(legend.position = c(.13, .95),
...
**** compare taner filters to bandpass filters
:PROPERTIES:
...
#+begin_src R :results output graphics file :file imgs/sol_SD_tie.png :width 2600 :height 700 :res 300
  best_age_optim |>
    ...
    filter(comb == "1:1") |>
    filter(detrend_type == "lin_scl_med") |> # ignore scl
    filter(site == "Zumaia") |>
