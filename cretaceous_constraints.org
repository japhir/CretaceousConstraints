  ...
  diffs <-
    ...
    mutate(num_cycles =
             ...
             case_when(
               ...
               site == "Zumaia" & depth_chunk == "depth >= 109.26" ~ 2.5,
               site == "Zumaia" & depth_chunk == "full record" ~ 9.5,
               site == "Sopelana" ~ 4.2,
               )
             ## length.out = 12 # this is to limit the total number of simulations
             ## )
             ) |>
    mutate(count_error = list(c(-0.5, 0, .5))) |>
    unnest(count_error) |>
    mutate(inv_sedrate = ((count_error + num_cycles) * 405) / depth_range) |>
    ## distinct(site, depth_chunk, inv_sedrate) # check if it worked
    ## unnest(cols = c(inv_sedrate)) |>
    mutate(floating_age = depth * inv_sedrate) |>
...
#+begin_src R :results value :colnames yes
  best <- msd |>
    ...
    tidylog::filter(RMSD == min(RMSD),
                    .by = c(proxy, site, comb, depth_chunk, count_error, #filt_env,
                            sol)) |>
...
#+RESULTS:
| proxy | detrend_type | comb | depth_chunk    | kpg_age | count_error | sol   |  RMSD |
|-------+--------------+------+----------------+---------+-------------+-------+-------|
| Lstar | lin_scl_fine |  1:1 | depth < 109.26 |   65900 |        -0.5 | ZB20a | 1.272 |
| Lstar | lin_scl_fine |  1:1 | depth < 109.26 |   65940 |         0.5 | La10b | 1.293 |
| Lstar | lin_scl_fine |  1:1 | depth < 109.26 |   65920 |         0.5 | ZB20b |  1.31 |
| Lstar | lin_scl_fine |  1:1 | depth < 109.26 |   65940 |        -0.5 | La10c | 1.331 |
| Lstar | lin_scl_fine |  1:1 | depth < 109.26 |   65920 |        -0.5 | ZB20c | 1.353 |
| Lstar | lin_scl_fine |  1:1 | depth < 109.26 |   65920 |        -0.5 | ZB20d | 1.357 |
| Lstar | lin_scl_fine |  1:1 | depth < 109.26 |   65940 |        -0.5 | ZB18a |  1.36 |
...
#+begin_src R :results output graphics file :file imgs/Zumaia-depth-RMSD-scores.png :width 2600 :height 1200 :res 300
  ...
  best |>
    ...
    geom_line(aes(group = paste(site, proxy, comb))) +
    geom_point(size = 2) +
...
#+begin_src R :results output graphics file :file imgs/Zumaia_MSD-overview.png :width 3400 :height 2600 :res 300
  msd |>
    ...
    geom_line(aes(group = paste(count_error,
                                inv_sedrate,
                                comb,
                                detrend_type),
...
#+begin_src R :results output graphics file :file imgs/Zumaia_ecc_RMSD.png :width 2600 :height 2200 :res 300
  my_filt <- \(d) d |>
                    filter(detrend_type == "lin_scl_fine")
  ...
  msd |>
    ...
    my_filt() |>
    ggplot(aes(x = kpg_age * 0.001, y = 1/inv_sedrate*100,
    ...
    facet_grid(cols = vars(proxy, site, depth_chunk),
               rows = vars(sol)) +
    scale_x_continuous(breaks = c(65.9 - 0.1, 65.9, 65.9 + .1)) +
    scale_fill_viridis_c() +
    scale_colour_viridis_c() +
  ...
  diffs |>
    ...
    ggplot(aes(x = age / 1e3,
               group = paste(comb, ## filt_env,
                             site, inv_sedrate, kpg_age, depth_chunk, #proxy_lag
                             count_error
                             ),
