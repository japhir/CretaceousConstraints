...
#+begin_src R :eval never
  ...
  rolling_age_optim <-
    ...
    write_rds("out/2024-01-17_rolling_age_optim_Sopelana_Lstar_lin-scl-fine.rds") # add highpass for 1:1 Lstar
  message("current time: ", lubridate::now())
...
#+begin_src R
  rolling_age_optim <-
    ...
    bind_rows(
      # comments above, where I write the rds files
...
#+begin_src R :results output graphics file :file imgs/new_rolling_improvement.png :width 3400 :height 1700 :res 300
  ...
  best_age_optim |>
    ...
    labs(x = "Astronomical solution", shape = "Age model") +
    ...
    geom_line(aes(y = RMSD_start,
                  group = paste(site, detrend_type))) +
...
**** plot of matched records and solutions for Zumaia Lstar 1:1
:PROPERTIES:
...
**** plot of matched records and solutions for Zumaia MS 1:1
:PROPERTIES:
:CREATED:  [2024-01-12 Fri 15:22]
:END:
...
**** plot of matched records and solutions for Zumaia d13C 1:1
:PROPERTIES:
...
#+begin_src R :results output graphics file :file imgs/Zumaia_Lstar_1-1_solutions_simple.png :width 2400 :height 2000 :res 300
  best_age_optim |>
    ...
    ## annotate("text", x = 66.3, y = 5, label = "RMSD", size = 3) +
    geom_text(aes(x = age * 1e-3 + 0.2, y = 4,
                  label = round(RMSD, 3)),
