...
#+begin_src R :results output graphics file :file imgs/bspline_scl.png :width 800
  filts <- bat_filters[bat_filters$proxy == "Lstar" &
                       bat_filters$depth_chunk == "full record" &
  ...
  pred |>
    ...
    coord_cartesian(xlim = range(the_knots$depth)) +
  geom_line(alpha=.3) +
  geom_line(aes(group = "x"), data = \(x) x |>
                                          nested_taner_filter(frequencies = filts, x = depth, y = Lstar_scl, nest = "site", roll = 1e10))
  ) / (
   pred |>
