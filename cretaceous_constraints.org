...
: Caused by warning:
: ! 5 unknown levels in `f`: gam_scl, hp_scl, lin_scl_rw, lin_scl_coarse, and value
...
#+begin_src R
  best_age_optim <- rolling_age_optim |>
    ...
    tidylog::filter(RMSD == min(RMSD),
                    .by = c(sol, site, proxy, detrend_type, depth_chunk, comb, window, roll
                            )) #|>
...
#+RESULTS:
: filter: removed 135 rows (62%), 82 rows remaining
...
: filter: removed 1,147 rows (1%), 109,420 rows remaining
: + + filter: removed 49 rows (70%), 21 rows remaining
